"""Create the private v1279 / iOS 378 Mac-source overlay package.
...
PREFIX = "SmallPhone_v1279_iOS378_Private/"
OUTPUT = ROOT.parent / "SmallPhone_v1279_iOS378_MacSource.zip"
...
def main() -> None:
    ...
    files["SOURCE_STATE.json"] = json.dumps({"sourceCommit":text(git("rev-parse","HEAD")).strip(),"upstreamBaseline":"a4195c9d42c9e048c0a473d978d181ec042500cf","houseSource":"df56e66a4f24efed42e4c37ef7d1908deb95df03","privateWeb":"v1279","privateIOS":"1.0.378 (378)","bridge":41,"macBuildVerified":False,"realIPhoneVerified":False,"kind":"Mac-source-not-IPA","preserved":["all-public-v1275-features","all-private-v1279-features","private-backup-4mib-object-chunks","private-backup-small-manifest-commit","private-backup-checksum-restore","private-backup-legacy-fallback","private-backup-single-upload-progress","web-cloud-existing-row-update","web-cloud-persistent-progress","private-backup-commit-timeout-1920s","online-memory-recall-controls","offline-memory-scroll-restore","cozy-private-native-bridge","diagnostic-copy-freshness","native-probe-lifecycle","house052-complete-assets","first-person-head-tracking","isolated-mobile-shader-warmup","warm-gpu-resource-release","female-standing-pose"]},ensure_ascii=False,indent=2).encode("utf-8")
    ...
    with TemporaryDirectory(prefix="private1279-verify-") as folder:
        with ZipFile(OUTPUT) as archive:

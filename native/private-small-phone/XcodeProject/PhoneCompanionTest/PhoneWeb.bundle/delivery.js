...
(function(){
  ...
  function requestRoleClarification(c,detail){
    ...
    var kind=detail.kind==='minimum_order'?'minimum_order':'options';
    ...
    roleSystemNotice(kind==='minimum_order'?'真实外卖已暂停：该门店未达到起送金额，等待你决定是否增加同款数量或换店。':'真实外卖已暂停：需要你确认平台可选规格后再继续，尚未加购或下单。','info');save();
    if(!c.blocked){var fact=kind==='minimum_order'?'[系统：这次真实外卖没有进入结算或付款，平台确认当前商品金额未达到门店起送要求，加购已停止或撤回。当前商品：'+offer.merchant+' / '+offer.name+'。真实限制：'+text(detail.reason,160)+'。请严格按你自己的完整人设和说话习惯，自然告诉'+S.me.name+'这个门店的起送限制，并只问一个必要问题：是否把同款增加到至少'+Math.max(2,+detail.minimumQuantity||2)+'份，还是换店。不要复述技术错误，不要声称已下单或付款。对方同意增加后系统会继续同一商品；对方要求换商品时系统会重新搜索。]':'[系统：这次真实外卖已安全暂停，尚未加购、下单或付款。当前商品：'+offer.merchant+' / '+offer.name+'。平台当前真实可选规格：'+clarificationOptions(offer)+'。未能继续的真实原因：'+text(detail.reason,160)+'。请严格按你自己的完整人设和说话习惯，只向'+S.me.name+'自然说明必要事实并问一个最必要的澄清问题；句式由你自己决定，不要照抄或复述技术错误，不要声称已经选好或下单。如果你想选热饮而当前商品没有热饮，只需自然问是否换一种，不要强迫对方接受冷饮。对方回答后系统会继续同一轮；若对方说出另一种商品，系统会放弃当前商品并重新搜索一次。]';scheduleReply(c.id,fact);}return true;
  }
  ...
  async function roleRequest(cid,query){
    ...
    var order=null;try{var picked;if(clarification&&clarification.offer&&clarification.offer.quoteExpiresAt>Date.now()+5000){var resumedOffer=safeOffer(clarification.offer),resumedChoice=await chooseOptions(c,query,resumedOffer,intent,clarification);if(clarification.kind==='minimum_order'&&clarification.minimumQuantity>1)resumedChoice.quantity=Math.max(resumedChoice.quantity,clarification.minimumQuantity);picked={offer:resumedOffer,choice:resumedChoice};}else{var offers=await realSearch(query,{roleId:cid});picked=await chooseOfferWithOptions(c,query,offers,intent);}var offer=picked.offer,choice=picked.choice;delete r.roleClarifications[cid];order=await createOrder(offer,{roleId:cid,quantity:choice.quantity,selectedOptions:choice.selectedOptions});pushRoleOrderCard(c,order);await payOrder(order);attempt.status='completed';attempt.endedAt=Date.now();save();resultReply(c,order,'');return true;}
  ...
  function rolePrompt(c){
    ...
    if(c&&r.roleClarifications[c.id]){var pendingClarification=r.roleClarifications[c.id],minimumOrder=pendingClarification.kind==='minimum_order';return minimumOrder?'\n\n# 真实外卖等待起送确认\n上一轮真实外卖已找到商品并尝试加购，但平台确认金额未达到门店起送要求，系统没有进入下单或付款。你已经或将要按自己的人设自然询问'+S.me.name+'：是否把同款增加到至少'+Math.max(2,+pendingClarification.minimumQuantity||2)+'份，还是换店。若对方明确同意增加同款，先用自己的语气简短表示会继续，紧接着单独输出 [真实外卖|继续确认起送]；系统会沿用同一商品并增加数量。若对方要求换商品或品牌，先自然回应，再单独输出 [真实外卖|换品:你根据对方新回答确定的具体搜索词]。若回答不明确，只自然追问一次。绝不能声称已经下单或付款，其他普通聊天照常。':'\n\n# 真实外卖等待规格确认\n上一轮真实外卖因为平台规格无法可靠判断而安全暂停，尚未加购、下单或付款。你已经或将要按自己的人设向'+S.me.name+'问一个必要问题。若对方这次明确回答了当前商品的选项、放宽要求，或说“都可以/你选”，先用你自己的语气简短表示会继续，紧接着单独输出 [真实外卖|继续确认规格]；系统会沿用同一商品和真实选项继续。若对方明确改要另一种商品或品牌，则先按你自己的口吻自然回应，再单独输出 [真实外卖|换品:你根据对方新回答确定的具体搜索词]，系统会放弃当前候选并重新搜索一次。若回答仍不明确，只自然追问一次，不要输出外卖标签。所有可见句式都由你按完整人设生成，不得使用固定客服话术；绝不能声称已经选好、下单或付款。其他普通聊天照常。';}

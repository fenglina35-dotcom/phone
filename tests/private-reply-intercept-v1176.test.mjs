...
test('private v1176 loads diagnostic parity between app core and theater extension',()=>{
  for(const file of ['index.html','小手机.html']){
    ...
    assert.match(html,/app\.js\?v=1176[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1176&r=v1176-private-intercept-parity-1"[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1176/);
  }
...
test('online, offline and cohab settings expose the last intercepted model text',()=>{
  assert.match(script,/window\.__NORTH_PRIVATE_REPLY_INTERCEPT__='v1176-private-intercept-parity-1'/);

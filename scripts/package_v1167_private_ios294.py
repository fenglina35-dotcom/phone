...
INSTALL_GUIDE_SOURCE = (
    SOURCE / "第二百九十四次安装_v1167_心动审判共同生活推文记忆修复_请先读.md"
)
DELIVERY = ROOT / "delivery-v1167-private294-heart-cohab-x-memory-candidate"
PACKAGE_NAME = "SmallPhone_v1167_HeartCohabXMemory_iOS294_MacReady"
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
USER_ZIP = ROOT.parent / "小手机_v1167_私人版_iOS294_心动审判共同生活推文记忆修复_Mac待编译源码包.zip"
...
if tracked_status and not ALLOW_DIRTY_PACKAGE:
    raise RuntimeError("tracked working tree is dirty; commit private iOS 294 first")
...
with tempfile.TemporaryDirectory(prefix="smallphone-v1167-ios294-", dir=ROOT) as temp:
    ...
    for token in [
        "window.__NORTH_SHELL_BUILD__='1167'",
        'private-runtime-diagnostics.js?v=294',
        "app.js?v=1167",
    ]:
    ...
    if "index.html?repair=1&v=1167" not in repair:
        raise RuntimeError("private repair page does not return to v1167")
    for destructive in [
        "localStorage.clear",
    ...
    for token in [
        "294-heart-cohab-x-memory-v1",
        "recovery.launch.peek",
        "recovery.launch.ack",
        "window.emergencyRestoreAll()",
    ]:
        if token not in overlay:
            raise RuntimeError(f"private 294 recovery marker missing: {token}")

    project = (
        staging / "PhoneCompanionTest.xcodeproj/project.pbxproj"
    ).read_text(encoding="utf-8")
    if project.count("CURRENT_PROJECT_VERSION = 294;") != 12:
        raise RuntimeError("private build 294 is not set on all targets/configurations")
    if project.count("MARKETING_VERSION = 1.0.294;") != 12:
        raise RuntimeError("private version 1.0.294 is not set consistently")
    ...
    if "私人 iOS 1.0.294 (294)" not in main_guide:
        raise RuntimeError("Mac guide does not identify private iOS 294")

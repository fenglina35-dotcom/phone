...
test('v1178 shared theater asset is byte-identical and loaded after app core',()=>{
  ...
  assert.match(webHtml,/app\.js\?v=1216[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1216&r=v1184-ios-web-crash-cohab-turn-keyboard-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1216[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1216[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1216&r=v1184-ios-web-crash-cohab-turn-keyboard-1"/);
...
test('private artifact identity advances independently to web core v1216 and iOS 1.0.341 (341)',()=>{
  assert.match(privateApp,/const APP_VER='v1216 · 私人外卖搜索修复版'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=333/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.341 \(341\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 341;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1\.0\.341;/g)||[]).length>=12);
});

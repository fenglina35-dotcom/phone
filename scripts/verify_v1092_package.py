...
package = root / "delivery-v1092-private217-final" / "SmallPhone_v1092_WeChatLoginPerformance_iOS217_MacReady.zip"
...
with ZipFile(package) as archive:
    ...
    guide = next((name for name in names if name.endswith("第二百一十七次安装_v1092_微信登录回话与交互流畅修复_请先读.md")), "")
    if not guide:
        raise RuntimeError("missing v1092 install readme")
    prefix = guide.split("第二百一十七次安装", 1)[0] + "PhoneCompanionTest/PhoneWeb.bundle/"
    ...
    for token in [
        "const APP_VER='v1092 · 微信登录回话与交互流畅修复版'",
    ...
    if "window.__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.217 (217)'" not in webview:
        raise RuntimeError("bundled private build marker mismatch")
    if project.count("CURRENT_PROJECT_VERSION = 217;") != 12 or project.count("MARKETING_VERSION = 1.0.217;") != 12:
...
print("v1092 package structure, byte identity, version and protected-route checks passed")

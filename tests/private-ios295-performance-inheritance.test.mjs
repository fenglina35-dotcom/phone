...
test('private identifiers remain v1273 and iOS 372 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1273'/);
  assert.match(index, /app\.js\?v=1273&r=v1273-private-sync-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=339/);
  assert.match(app, /APP_VER='v1273 · 私人版温馨小家与流畅度整合'/);
  assert.match(overlay, /336-daily-file-backup/);
  assert.match(webview, /1\.0\.372 \(372\)/);
  assert.match(bridge, /private static let build = "1\.0\.372 \(372\)"/);
  assert.match(bridge, /static let contractVersion = 38/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 372;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.372;/g) || []).length, 12);
...
test('Mac guide identifies the current private v1273 iOS372 source', () => {

    ...
    while "\nruntime_globals =" in code:
        if current_path.name == "package_v1187_private_ios313.py":
            ...
            code = code.replace(
                ...
                'if "html.north-native-app .phone:has(.offinput)" not in entry:\n'
                '        raise RuntimeError("private common-life caret coordinate rule is missing")',
            )
        prefix = code.rsplit("\nruntime_globals =", 1)[0]
        namespace = {
...
for old, new in v1188_replacements:
    if old not in code:
        raise RuntimeError(f"v1188 packaging baseline token missing: {old}")
    code = code.replace(old, new)

replacements = [
    (
        "第三百一十四次安装_v1188_线下原生聚焦修复_请先读.md",
        "第三百一十五次安装_v1189_共同生活键盘坐标修复_请先读.md",
    ),
    (
        "delivery-v1188-private314-offline-native-focus-release",
        "delivery-v1189-private315-cohab-keyboard-coordinate-release",
    ),
    (
        "SmallPhone_v1188_OfflineNativeFocus_iOS314_MacReady",
        "SmallPhone_v1189_CohabKeyboardCoordinate_iOS315_MacReady",
    ),
    (
        "小手机_v1188_私人版_iOS314_线下原生聚焦修复_Mac待编译源码包.zip",
        "小手机_v1189_私人版_iOS315_共同生活键盘坐标修复_Mac待编译源码包.zip",
    ),
    ("private-web=v1188", "private-web=v1189"),
    ("ios=1.0.314 (314)", "ios=1.0.315 (315)"),
    ("1.0.314 (314)", "1.0.315 (315)"),
    ("CURRENT_PROJECT_VERSION = 314;", "CURRENT_PROJECT_VERSION = 315;"),
    ("MARKETING_VERSION = 1.0.314;", "MARKETING_VERSION = 1.0.315;"),
    (
        "smallPhone.webContentTerminationTimes.v23.build314",
        "smallPhone.webContentTerminationTimes.v24.build315",
    ),
    (
        "314-private-v1188-offline-native-focus-1",
        "315-private-v1189-cohab-keyboard-coordinate-1",
    ),
    ("private-runtime-diagnostics.js?v=314", "private-runtime-diagnostics.js?v=315"),
    ("window.__NORTH_SHELL_BUILD__='1188'", "window.__NORTH_SHELL_BUILD__='1189'"),
    (
        "app.js?v=1188&r=v1188-private-offline-native-focus-1",
        "app.js?v=1189&r=v1189-private-cohab-keyboard-coordinate-1",
    ),
    ("private-reply-intercept.js?v=1188", "private-reply-intercept.js?v=1189"),
    ("cohab-theater.js?v=1188", "cohab-theater.js?v=1189"),
    ("bead-studio.js?v=1188", "bead-studio.js?v=1189"),
    ("delivery.js?v=1188", "delivery.js?v=1189"),
    ("index.html?repair=1&v=1188", "index.html?repair=1&v=1189"),
    (
        "APP_VER='v1188 · 私人线下原生聚焦修复版'",
        "APP_VER='v1189 · 私人共同生活键盘坐标修复版'",
    ),
]

for old, new in replacements:
    if old not in code:
        raise RuntimeError(f"v1189 packaging template token missing: {old}")
    code = code.replace(old, new)
...
code = code.replace(
    'if "APP_VER=\'v1189 · 私人共同生活键盘坐标修复版\'" not in public_app:',
...
for token in required_packager_tokens:
    if token not in code:
        raise RuntimeError(f"materialized v1189 packager token missing: {token}")
...
if ".ignoresSafeArea(.keyboard, edges: .bottom)" not in private_root:
    raise RuntimeError("stable private keyboard host contract is missing")
if "function offComposerPinLatest(target)" in private_app:
    raise RuntimeError("failed v1187 touch-time scroll anchor remains")
if "_offComposerGuardUntil=Date.now()+1600" not in private_app:
    raise RuntimeError("common-life touch guard duration is missing")
if "stopImmediatePropagation" not in private_app:
    raise RuntimeError("common-life click fall-through guard is missing")
for entry in private_entries:
    ...
    if "html.north-native-app .phone:has(.offinput)" not in entry:
        raise RuntimeError("private common-life caret coordinate rule is missing")
    if "window.__NORTH_SHELL_BUILD__='1189'" not in entry:
        raise RuntimeError("private v1189 entry identity is missing")
for forbidden in [

...
test('private identifiers remain v1278 and iOS 377 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1278'/);
  assert.match(index, /app\.js\?v=1278&r=v1278-private-backup-storage-1/);
  ...
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 377;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.377;/g) || []).length, 12);
...
test('Mac guide identifies the current private v1278 iOS377 source', () => {

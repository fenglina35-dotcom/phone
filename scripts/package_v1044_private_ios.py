...
DELIVERY = ROOT / "delivery-v1044"
PACKAGE_NAME = "SmallPhone_v1044_RemoteCaptionDeliverySafety_iOS164_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百六十四次安装_v1044_远控字幕与外卖风控降扰_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1044":
...
for required_text in [
    "APP_VER='v1044 · 远控字幕与外卖风控降扰修复版';",
    "remoteControlEnsureRoleCaption",
    "remoteControlRoleEvidence",
    "nfRoleSearchMatches",
    "chat-quote-pending",
    ...
    if required_text not in app_text:
        raise RuntimeError(f"v1044 bundled repair is missing: {required_text}")

delivery_text = (bundle / "delivery.js").read_text(encoding="utf-8")
for required_text in ["roleSystemNotice", "lastRoleNotice", "真实订单不存在"]:
    if required_text not in delivery_text:
        raise RuntimeError(f"v1044 bundled delivery repair is missing: {required_text}")

if "<string>1044</string>" not in (bundle / "Info.plist").read_text(encoding="utf-8"):
    raise RuntimeError("PhoneWeb.bundle Info.plist is not v1044")

project_text = (STAGING / "PhoneCompanionTest.xcodeproj" / "project.pbxproj").read_text(encoding="utf-8")
if project_text.count("CURRENT_PROJECT_VERSION = 164;") != 12:
    raise RuntimeError("private iOS build number is not consistently 164")
if project_text.count("MARKETING_VERSION = 1.0.164;") != 12:
    raise RuntimeError("private iOS marketing version is not consistently 1.0.164")

webview_text = (STAGING / "PhoneCompanionTest" / "LocalPhoneWebView.swift").read_text(encoding="utf-8")
if "__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.164 (164)'" not in webview_text:
    raise RuntimeError("private build marker is not 1.0.164 (164)")

...
test('private entries carry diagnostics 336 and every iOS target carries build 356', () => {
  ...
  assert.match(bridge, /1\.0\.392 \(392\)/);
  assert.equal(
    (project.match(/CURRENT_PROJECT_VERSION = 392;/g) || []).length,
    12
  );
  assert.equal(
    (project.match(/MARKETING_VERSION = 1.0.392;/g) || []).length,
    12
  );
});

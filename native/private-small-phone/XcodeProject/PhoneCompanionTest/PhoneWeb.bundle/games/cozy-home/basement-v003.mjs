import {refineRug021} from './comfort-v021.mjs';
import {refineCraftObjects} from './prop-details-v016.mjs';
...
export async function finishBasement(house,world){
 ...
 const metalCanvas=document.createElement('canvas');metalCanvas.width=metalCanvas.height=256;const mq=metalCanvas.getContext('2d');mq.fillStyle='#888';mq.fillRect(0,0,256,256);let ms=901;for(let i=0;i<6500;i++){ms=(ms*1664525+1013904223)>>>0;const x=ms%256;ms=(ms*1664525+1013904223)>>>0;mq.fillStyle=i%2?'#929292':'#7e7e7e';mq.fillRect(x,ms%256,1,1);}const mt=new T.CanvasTexture(metalCanvas);mt.wrapS=mt.wrapT=T.RepeatWrapping;iron.bumpMap=mt;iron.bumpScale=.0006;iron.roughness=.39;iron.metalness=.72;
 ...
 // The 1.12 m gate uses the right hinge and opens away from the player; manual control.
 ...
 const hinge=new T.Group();hinge.name='Manual bidirectional iron gate';hinge.position.set(21.22,0,12.10);hinge.rotation.y=Math.PI;cell.add(hinge);rail(hinge,.56,0,1.06);
 ...
 box(hinge,'Latch plate',1.0,1.08,-.042,.12,.20,.025,iron,.015);
 tube(hinge,'Gate pull handle',[[1,1.03,-.035],[1,1.04,-.105],[1,1.17,-.105],[1,1.18,-.035]],.013,brass);
 for(const x of[17.60,20.10,21.22,21.65]){box(cell,'Bolted iron foot plate',x,.027,12.10,.14,.025,.17,iron,.009);for(const dx of[-.046,.046])for(const dz of[-.057,.057])cyl(cell,'Frame anchor bolt',x+dx,.046,12.10+dz,.008,.008,.016,brass);for(const y of[.16,2.32]){const o=mesh(cell,'Welded rounded bar collar',new T.TorusGeometry(.017,.004,6,20),iron,[x,y,12.10]);o.rotation.x=Math.PI/2;}}
 ...
 let open=false,gateAngle=0,gateSign=1,locked=false;
 function toggleGate(player){
  if(locked)return {changed:false,message:"禁闭室门已从外面锁上"};
  if(open&&Math.hypot(player.x-20.66,player.z-12.1)<.42)return {changed:false,message:'请稍离门口再关门'};
  if(!open&&Math.abs(gateAngle)<.02)gateSign=player.z>=12.1?-1:1;
  open=!open;return {changed:true,message:open?'铁门打开':'铁门关闭'};
 }
 function syncGateCollider(){const c=world.colliders.find(c=>c.id==='cell gate');if(c){const x=21.22+Math.cos(Math.PI+gateAngle)*1.12,z=12.10-Math.sin(Math.PI+gateAngle)*1.12;Object.assign(c,{x:Math.min(21.22,x)-.045,z:Math.min(12.10,z)-.045,w:Math.abs(x-21.22)+.09,d:Math.abs(z-12.10)+.09});}}
 function automaticGate(dt,player){const previous=gateAngle,target=open?gateSign*Math.PI/2:0;gateAngle=T.MathUtils.damp(gateAngle,target,7,dt);if(Math.abs(target-gateAngle)<.0002)gateAngle=target;hinge.rotation.y=Math.PI+gateAngle;syncGateCollider();return Math.abs(gateAngle-previous)>.00001;}
 ...
 const rug=mesh(cell,'Thick rounded plush rug',new T.SphereGeometry(1,128,48),pileMat,[19.61,.030,11.22]);rug.scale.set(1.82,.05,.77);
 refineRug021(rug,pileMat);
 ...
 for(const [i,toy]of [teddy,melody,smaller].entries()){root.updateMatrixWorld(true);let b=new T.Box3().setFromObject(toy);toy.position.x += [18.15,19.10,20.03][i]-(b.min.x+b.max.x)/2;toy.position.z+=9.69-b.min.z;toy.position.y=.004;root.updateMatrixWorld(true);b=new T.Box3().setFromObject(toy);addCollider(['large teddy','melody plush','small teddy'][i],b.min.x,b.min.z,b.max.x-b.min.x,b.max.z-b.min.z,b.max.y-FLOOR);toy.userData.wallClearance=.06;toy.userData.support="oak floor at 0.004 m";}
 const oldShadows=[];cell.traverse(o=>{if(o.name==='Plush contact shadow on rug')oldShadows.push(o)});oldShadows.forEach(o=>o.removeFromParent());
 for(const toy of toys.children){root.updateMatrixWorld(true);const b=new T.Box3().setFromObject(toy),c=b.getCenter(new T.Vector3());const sh=mesh(cell,'Grounded plush contact shadow',new T.PlaneGeometry((b.max.x-b.min.x)*1.08,(b.max.z-b.min.z)*.8),new T.MeshBasicMaterial({map:shadowTex,transparent:true,depthWrite:false}),[c.x,.0035,c.z+.04]);sh.rotation.x=-Math.PI/2;sh.castShadow=false;}
 ...
 for(const o of [...cell.children])if(/Single small yarn ball|Wound wool strand|Loose resting yarn end/.test(o.name))o.position.add(new T.Vector3(-1.68,-.065,-1.23));
 const yarnTail=cell.getObjectByName('Loose resting yarn end');if(yarnTail){const a=yarnTail.geometry.attributes.position;for(let i=0;i<a.count;i++)a.setY(i,Math.max(.070,a.getY(i)));yarnTail.geometry.computeVertexNormals();}
 ...
 const detailAudit=refineCraftObjects(fixtures,desk);desk.getObjectByName('Resting lamp cable')?.removeFromParent();
 const parts=[];
 ...
 house.basementFinish={root,cell,toys,hinge,detailAudit,update:automaticGate,bindLighting,toggleGate,setLocked:v=>{locked=!!v;},state:()=>({locked,open,gateAngle,manual:true,plan:basementPlan,parts:parts.length,cellContents:root.userData.cellContents}),nearGate:p=>p.y<-2.5&&Math.hypot(p.x-20.66,p.z-12.1)<1.65};

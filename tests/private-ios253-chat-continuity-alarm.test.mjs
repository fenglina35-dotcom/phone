...
test('thermal pressure and repeated termination stop WebKit self-reload loops', () => {
  ...
  assert.match(webView, /webContentTerminationTimes\.v5\.build293/);
...
test('both private entry files and every iOS target carry build 293', () => {
  assert.equal(index, alias);
  assert.match(index, /private-runtime-diagnostics\.js\?v=293/);
  assert.match(overlay, /293-heartquiz-batches-v1/);
  assert.match(webView, /1\.0\.293 \(293\)/);
  assert.match(bridge, /1\.0\.293 \(293\)/);
  assert.equal(
    (project.match(/CURRENT_PROJECT_VERSION = 293;/g) || []).length,
    12
  );
  assert.equal(
    (project.match(/MARKETING_VERSION = 1\.0\.293;/g) || []).length,
    12
  );
});

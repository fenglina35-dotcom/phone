...
for token in [
    ...
    if token not in app:
        raise RuntimeError(f"v1060 app feature missing: {token}")
for token in [
    "tryClarificationFallback",
    "clarificationAcceptedAddons",
    "clarificationAddonCandidates",
    "roleRequestIntent",
    "realSearch",
    "createOrder",
]:
    if token not in delivery:
        raise RuntimeError(f"v1060 delivery feature missing: {token}")
...
source = (
    ...
    .replace("1058", "1060")
    .replace("181", "183")
    .replace("第一百八十一次安装", "第一百八十三次安装")
    .replace("CohabStorageTransferOfflineInput", "BusyDeliveryClarification")
    .replace("共同生活存储散热转账回复与约会输入修复", "微信忙碌与外卖澄清续接修复")
    .replace("共同生活通话、存储散热、转账回复与约会输入修复版", "微信忙碌与外卖澄清续接修复版")
)

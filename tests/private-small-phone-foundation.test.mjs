...
test('private app loads bundled phone resources instead of a remote shell', () => {
  ...
  assert.match(webView, /\n      window\.__SMALL_PHONE_PRIVATE_BUILD__ = '1\.0\.308 \(308\)'/);
...
test('real Mac project keeps all Screen Time targets and becomes 小手机', () => {
  ...
  assert.match(project, /CURRENT_PROJECT_VERSION = 308;/);
  assert.match(project, /MARKETING_VERSION = 1\.0\.308;/);
...
test('private app isolates role audio from recognition and reuses the proven web keyboard flow', () => {
  ...
  assert.doesNotMatch(privateRoot, /\.ignoresSafeArea\(\.keyboard, edges: \.bottom\)/,
    'the private WKWebView must resize with the native keyboard like the shared web composer');

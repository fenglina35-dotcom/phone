...
test('private identifiers remain v1325 and iOS 394 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1325'/);
  assert.match(index, /app\.js\?v=1325&r=v1325-private-imsg-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=339/);
  assert.match(app, /APP_VER='v1325 · 电话频率入口与手机显示修复'/);
  assert.match(overlay, /336-daily-file-backup/);
  assert.match(webview, /1\.0\.396 \(396\)/);
  assert.match(bridge, /private static let build = "1\.0\.396 \(396\)"/);
  assert.match(bridge, /static let contractVersion = 41/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 396;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.396;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1324 · 电话频率入口与手机显示修复'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1324'/);
...
test('Mac guide identifies the current private v1325 iOS396 source', () => {

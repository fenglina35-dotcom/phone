...
const WX_HELP=[
 ...
 {title:'角色心情气泡',aliases:['心情标签','心声标签','心情气泡','心声气泡','角色心情','角色心声','内心想法'],answer:'在“微信 → 我 → 设置 → 聊天 → 心情气泡”开启。开启后，角色有新的心情或心声内容时，会固定显示在角色聊天顶部；当前还没有心情内容时不会显示空白气泡。点气泡可以查看完整内容。'},
 ...
 {title:'创建与管理角色',aliases:['创建角色','新建角色','角色资料','角色人设','修改人设','角色头像','角色备注','删除角色'],answer:'在“微信 → 通讯录”点右上角添加按钮可新建角色；已有角色进入聊天后点右上角“⋯”，可修改头像、名字、备注、人设、语音、气泡和角色功能。删除属于不可恢复操作，先备份聊天与资料。'},
 {title:'世界书',aliases:['世界书','设定集','背景设定','共享设定','角色世界观'],answer:'从小手机主屏打开“世界书”，可保存地点、组织、规则和共同背景，再按需要关联角色。世界书用于补充角色设定，不会代替角色自己的核心人设或自动公开任何密钥。'},
 {title:'回复方式与手动回复',aliases:['手动回复','让ta回','让他回','自动回复','回复中','角色不回复','主动回复'],answer:'在“小手机主屏 → 设置 → 角色、时间与消息”管理各场景的手动回复。开启后，微信、游戏、角色扮演或线下约会会显示“让ta回/让TA回应”；关闭对应场景后才按该场景的自动流程回复。生成失败时不会用固定假话冒充角色。'},
 {title:'聊天 API 多路线',aliases:['api路线','多路线','路线一','路线二','路线三','路线四','切换模型路线','辅助模型','副模型'],answer:'在“小手机主屏 → 设置 → 网络连接”可保存四条聊天 API 路线，每条同时保存主模型和辅助模型。聊天或游戏中的路线按钮只切换已经配置好的路线，从下一次回复起生效，不会中断正在生成的回复。'},
 {title:'联网搜索',aliases:['联网搜索','网络搜索','搜索接口','角色上网','联网模型'],answer:'在“小手机主屏 → 设置 → 网络连接”配置联网搜索。它用于确实需要最新网页信息的功能；普通角色聊天不会因为开启联网就自动暴露 Key，也不能绕过网站登录、支付或平台风控。'},
 {title:'图片识别',aliases:['图片识别','识别图片','看图片','看照片','摄像头识图','视觉模型','图片理解'],answer:'进入“小手机主屏 → 设置 → 图片与识别”配置识图地址、Key 和模型并先测试。聊天里发送图片后，只有支持图片的线路才能理解；配置失败时不会假装已经看见图片。'},
 {title:'语音转文字',aliases:['语音转文字','语音识别','录音转文字','whisper','字幕提取','提取字幕'],answer:'外置语音识别在“小手机主屏 → 设置 → 声音与通话”相关入口配置；私人 AI 账户也可能提供内置识别。放映室提取字幕会使用已配置的识别线路，大文件处理时需保持页面开启并查看任务进度。'},
 {title:'电话与音视频通话',aliases:['电话','打电话','语音通话','视频通话','来电铃声','通话背景','视频识图','屏幕共享'],answer:'从主屏“电话”或角色聊天的“＋ → 语音通话/视频通话”发起。全局声音与通话设置在“小手机主屏 → 设置 → 声音与通话”，角色来电铃声可在个人资料设置；视频识图和屏幕共享是否可用还取决于私人 App 权限与当前模型能力。'},
 {title:'主屏外观与布局',aliases:['主屏壁纸','桌面壁纸','app图标','软件图标','主屏组件','桌面布局','拖动软件','移动app','透明玻璃','主屏时间'],answer:'在“小手机主屏 → 设置 → 外观与主屏”调整壁纸、主题、App 图标、组件和时间显示。主屏长按 App 或组件可拖动换位、换页；完整桌面顺序和数据工具在“设置 → 授权与数据 → 桌面布局”。'},
 {title:'锁屏与屏保',aliases:['锁屏','屏保','锁屏壁纸','屏保时间','屏保箭头','息屏'],answer:'锁屏和屏保外观在小手机主设置的外观相关页面调整。锁屏壁纸、时间颜色与主屏设置相互独立或按页面说明联动；屏保只影响显示，不会删除聊天、角色或后台任务。'},
 {title:'微信聊天操作',aliases:['微信聊天','引用消息','长按消息','删除消息','转发消息','发图片','发位置','红包','转账','名片'],answer:'进入角色聊天后，长按消息可使用引用、收藏、删除或转发等现有操作；底部“＋”包含相册、通话、位置、红包、转账、文件和名片等功能。模拟红包、转账和钱包只改小手机内部数据，不是真实支付。'},
 {title:'音乐与一起听',aliases:['音乐','一起听','歌单','添加歌曲','歌词','音乐背景','网易云'],answer:'从主屏打开“音乐”，可搜索或添加歌曲、管理歌单、歌词、封面和播放背景，并邀请角色一起听。一起听有独立聊天记录，会进入角色上下文，但不会伪装成普通微信聊天消息。'},
 {title:'放映室',aliases:['放映室','一起看','一起读','看电影','看视频','读小说','弹幕','陪看','陪读'],answer:'从主屏打开“放映室”，选择角色后可一起看本地视频或读文本。字幕、识图、语音和自动评论分别受放映室设置与模型能力控制；角色只能依据已经播放或读到的内容回应，不能假装看过后续剧情。'},
 {title:'游戏大厅',aliases:['游戏大厅','游戏返回','你画我猜','海龟汤','谁是卧底','真心话','电子宠物','游戏上下文'],answer:'从主屏或“微信 → 发现 → 游戏”进入游戏大厅。选择游戏和角色后开始，未结束的部分会保留草稿；从微信发现进入时，游戏页左上角返回应回到微信发现，而不是主屏。'},
 {title:'线下约会与共同生活',aliases:['线下约会','共同生活','线下模式','约会','婚礼仪式','一起生活','当面说'],answer:'从主屏“线下约会”进入，可选择共同生活、婚礼仪式或单次约会。这里的场景、动作和对话独立发生，结束后按真实记录形成角色记忆；“当面说”使用系统原生输入框，不能用美化光标破坏键盘输入。'},
 {title:'角色扮演与剧情应用',aliases:['角色扮演','规则怪谈','惊悚抉择','剧情模式','小剧场'],answer:'主屏的“角色扮演”“规则怪谈”“惊悚抉择”是独立剧情空间，可选择角色和回复方式。剧情记录、退出和记忆按各页面规则保存；它们不会覆盖普通微信聊天，也不会强行改变角色人设。'},
 {title:'情侣空间与远程控制',aliases:['情侣空间','查他手机','查手机','远程控制','锁软件','解锁软件','情侣绑定'],answer:'“情侣空间”管理情侣关系和已授权互动；“查他手机”与远程控制只在双方已配置、已授权的模拟范围内工作。锁定 App、查看状态等操作不会取得真实支付密码、系统相册或未授权的设备数据。'},
 {title:'任务便签、日历与信箱',aliases:['任务便签','任务','便签','日历','信箱','邮件'],answer:'主屏“任务便签”用于记录和管理任务，“日历”查看日期与相关记录，“信箱”接收小手机内生成的邮件内容。它们使用小手机本地数据，不会自动读取现实邮箱或系统日历。'},
 {title:'购物、外卖与浏览器',aliases:['购物','购物软件','浏览器','网页','打开网站','虚拟外卖','真实外卖开关'],answer:'主屏“购物”和未开启真实模式的外卖属于小手机内部体验；浏览器用于打开网页。真实外卖必须单独连接真实服务，开启后不会在失败时回退成虚拟订单；现实平台的登录、地址、优惠和付款仍受平台实际能力限制。'},
 {title:'X、抖音与朋友圈内容',aliases:['抖音','短视频','x动态','微博','发动态','角色动态','评论动态'],answer:'主屏的 X、抖音和微信朋友圈分别保存各自的动态与互动。角色回复必须来自当前真实模型线路；生成失败时不制造固定角色回复。删除朋友圈相册中的图片会删除对应整条朋友圈。'},
 {title:'手机授权与私人 App',aliases:['手机授权','邀请码','扫脸恢复','指纹恢复','passkey','私人app','安装包','换浏览器'],answer:'在“小手机主屏 → 设置 → 授权与数据”查看手机授权、手机号账号和恢复方式。已经绑定的设备优先使用系统扫脸或指纹恢复；安装包版本、网页核心版本和原生桥版本是三项不同信息，不能只看其中一个判断是否更新成功。'},
 {title:'存储、清理与数据安全',aliases:['存储空间','清理缓存','清空聊天','清空数据','数据安全','缓存垃圾','空间不足'],answer:'在“小手机主屏 → 设置 → 授权与数据”查看存储、备份和清理工具。清缓存、清聊天、清空全部数据的范围不同，执行前必须看页面说明并先备份；不要通过客服发送 Key、Token、支付密码、银行卡资料或内部源码。'},
 {title:'全部功能概览',aliases:['全部功能','功能列表','有哪些功能','软件有哪些','所有功能','小手机能做什么'],answer:'小手机主要包含：微信与角色聊天、朋友圈和真人好友、电话与音视频、世界书、浏览器、查他手机、购物、外卖、日历、X、抖音、情侣空间、任务便签、游戏大厅、信箱、线下约会与共同生活、角色扮演、云程、音乐、放映室、规则怪谈、惊悚抉择、AI 账户，以及模型、语音、识图、外观、授权和备份设置。你可以直接说某个功能名，我会本地秒答入口和用法。'},
...
const WX_SUPPORT_QUICK=['API 密钥怎么配置','角色语音在哪里设置','聊天气泡怎么换','角色心情标签在哪里开启','真实外卖怎么用','如何扫码加好友','收藏语音在哪里','如何备份数据','后台主动消息怎么开'];

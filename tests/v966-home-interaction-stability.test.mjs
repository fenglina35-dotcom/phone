...
test('v1123 web keeps private 1.0.243 compatibility',()=>{
  assert.match(app,/APP_VER='v1123 · 模型路线真实诊断版'/);
  assert.match(html,/__NORTH_SHELL_BUILD__='1123'/);
  assert.match(sw,/BUILD='1123'/);
  assert.match(plist,/<string>1123<\/string>/);
  assert.equal((project.match(/CURRENT_PROJECT_VERSION = 249;/g)||[]).length,12);
  assert.equal((project.match(/MARKETING_VERSION = 1\.0\.249;/g)||[]).length,12);
  assert.match(native,/1\.0\.249 \(249\)/);
});

...
package = root / "delivery-v1093-private218-final" / "SmallPhone_v1093_WeChatLoginReturn_iOS218_MacReady.zip"
...
with ZipFile(package) as archive:
    ...
    guide = next((name for name in names if name.endswith("第二百一十八次安装_v1093_微信登录退出回话修复_请先读.md")), "")
    if not guide:
        raise RuntimeError("missing v1093 install readme")
    prefix = guide.split("第二百一十八次安装", 1)[0] + "PhoneCompanionTest/PhoneWeb.bundle/"
    ...
    for token in [
        "const APP_VER='v1093 · 微信登录退出回话修复版'",
        "function wxLoginCompletionChannel",
        "function wxLoginCompletionRepairPrompt",
        "function wxLoginCompletionReplyValid",
        "if(wxLoginCompletionFeature(note))return scheduleReply(id,note,onDone,aid)",
    ]:
        if token not in app:
            raise RuntimeError(f"bundled app fix missing: {token}")
    if "window.__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.218 (218)'" not in webview:
        raise RuntimeError("bundled private build marker mismatch")
    if project.count("CURRENT_PROJECT_VERSION = 218;") != 12 or project.count("MARKETING_VERSION = 1.0.218;") != 12:
...
print("v1093 package structure, byte identity, version and login-return checks passed")

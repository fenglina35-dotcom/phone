...
test('v1178 has a unique visible identity across every public entry and cache layer',()=>{
  assert.match(app,/__NORTH_SHELL_BUILD__!==\'1195\'/);
  assert.match(app,/APP_VER='v1195 · 情侣空间触发计数版'/);
  assert.match(shell,/__NORTH_SHELL_BUILD__='1195'/);
  assert.match(shell,/app\.js\?v=1195&r=v1195-couple-watch-trigger-1/);
  assert.match(index,/小手机\.html\?v=1195/);
  assert.match(repair,/小手机\.html\?v=1195/);
  assert.match(worker,/const BUILD='1195'/);
  assert.match(worker,/north-shell-v1195-couple-watch-trigger-1/);
  assert.match(hotfix,/sw\.js\?v=1195&r=v1195-couple-watch-trigger-1/);

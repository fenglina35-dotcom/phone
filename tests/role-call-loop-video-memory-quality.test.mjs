...
test('shared call and memory sources stay synchronized during the scoped web-only import repair',()=>{
  ...
  assert.equal(webVersion,1249,'the scoped browser release must keep its assigned web version');
  assert.equal(privateVersion,1248,'the private bundle must carry the release identity assigned to it this round');

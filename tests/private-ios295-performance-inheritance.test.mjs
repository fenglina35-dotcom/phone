...
test('private identifiers advance to v1248 and iOS 359 while public stays v1248', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1248'/);
  assert.match(index, /app\.js\?v=1248&r=v1248-cohab-schedule-churn-1/);
  ...
  assert.match(webview, /1\.0\.359 \(359\)/);
  assert.match(bridge, /private static let build = "1\.0\.359 \(359\)"/);
  assert.match(bridge, /static let contractVersion = 38/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 359;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.359;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1248 · 共同生活作息卡顿修复'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1248'/);
...
test('Mac guide identifies the current private v1248 iOS359 source', () => {

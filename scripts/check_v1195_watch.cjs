...
const server=http.createServer((req,res)=>{
 ...
 if(url.pathname==='/mock/chat/completions'){
  let body='';req.on('data',chunk=>body+=chunk);req.on('end',()=>{
   ...
   setTimeout(()=>{res.setHeader('Content-Type','application/json');res.end(JSON.stringify({choices:[{message:{content:'[内心|刚才看见他找朋友，心里有点好奇。]\n刚才去找朋友了呀。\n还挺投入的。\n看到什么有趣的了吗？\n也和我说说。'},finish_reason:'stop'}]}));},2500);
  });return;
 }
...
(async()=>{
 ...
 try{
  await Promise.all([{kind:'chat',privateApp:false},{kind:'douyin',privateApp:false},{kind:'chat',privateApp:true},{kind:'douyin',privateApp:true}].map(async ({kind,privateApp})=>{const caseId=kind+(privateApp?'-private':'-web');
   ...
   await page.evaluate(({kind,origin,caseId})=>{
    ...
    S.settings.chatRoutes=[{base:origin+'/mock',key:'test-only',model:caseId,temp:.8,maxTokens:900}];S.settings.chatRouteActive=0;
   ...
   assert.equal(requests.filter(x=>x.model===caseId).length,0);
   ...
   // Stay on observed page until delivery.
   ...
   assert.equal(requests.filter(x=>x.model===caseId).length,1);
   assert.match(requests.find(x=>x.model===caseId).event,/第 1 次/);
   if(kind==='douyin')assert.match(requests.find(x=>x.model===caseId).event,/抖音/);
   ...
   await page.evaluate(()=>openChat(S.couple.cid));
   assert.equal(await page.locator('#chatbg').getByText('刚才去找朋友了呀。',{exact:true}).count(),1);
   assert.equal(await page.locator('#chatMoodBar').isVisible(),true);
   assert.match(await page.locator('#chatMoodText').innerText(),/刚才看见他找朋友/);
   await page.evaluate(()=>{const c=getC(S.couple.cid);c.innerThoughtMissingAt=Date.now();save();home();openChat(c.id);});
   assert.equal(await page.locator('#chatMoodBar').isVisible(),true);
   assert.deepEqual(errors,[]);
   console.log(JSON.stringify({kind,privateApp,elapsed,modelRequests:1,delivered:4,canonicalThought:true,thoughtSurvivesReopen:true,version:result.version,pageErrors:errors}));
   await context.close();
  }));

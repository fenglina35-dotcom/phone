...
test('private identity remains v1283 and iOS 380 while public advances independently', () => {
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!=='1283'/);
  assert.match(privateApp, /APP_VER='v1283 · 私人内部标记与朋友圈封面修复'/);
  for (const html of [privateIndex, privateAlias]) {
    assert.match(html, /window\.__NORTH_SHELL_BUILD__='1283'/);
    assert.match(html, /app\.js\?v=1283&r=v1283-private-app-decision-1/);
    assert.match(html, /private-runtime-diagnostics\.js\?v=339/);
  }
  assert.match(privateRepair, /index\.html\?repair=1&v=1283/);
  ...
  assert.match(publicIndex, /window\.__NORTH_SHELL_BUILD__='1282'/);

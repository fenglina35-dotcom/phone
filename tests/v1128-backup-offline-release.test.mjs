...
test('v1178 has a unique visible identity across every public entry and cache layer',()=>{
  assert.match(app,/__NORTH_SHELL_BUILD__!==\'1236\'/);
  assert.match(app,/APP_VER='v1236 · 恢复完整备份自动下载'/);
  assert.match(shell,/__NORTH_SHELL_BUILD__='1236'/);
  assert.match(shell,/app\.js\?v=1236&r=v1236-backup-auto-download-1/);
  assert.match(index,/小手机\.html\?v=1236/);
  assert.match(repair,/小手机\.html\?v=1236/);
  assert.match(worker,/const BUILD='1236'/);
  assert.match(worker,/north-shell-v1236-backup-auto-download-1/);
  assert.match(hotfix,/sw\.js\?v=1236&r=v1236-backup-auto-download-1/);

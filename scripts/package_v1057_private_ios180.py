...
DELIVERY = ROOT / "delivery-v1057-private180-final"
PACKAGE_NAME = "SmallPhone_v1057_CohabPetBatteryTransfer_iOS180_MacReady"
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百八十次安装_v1057_共同生活宠物电量与微信转账整合_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1057-private180-final":
...
with tempfile.TemporaryDirectory(prefix="smallphone-v1057-", dir=ROOT) as temp:
    ...
    for token in [
        "APP_VER='v1057 · 共同生活宠物电量与转账整合版';",
        "cohabActionTap", "webBatteryResume", "renderTransferDetail",
        "PRIVATE_MIRROR_MODE='private-primary'", "remoteControlRoleReaction",
    ]:
        if token not in app_text:
            raise RuntimeError(f"v1057 bundled app feature missing: {token}")
    ...
    for token in ["_petSceneTapBlockUntil", "blockSceneTap", "petBowl"]:
        if token not in pet_text:
            raise RuntimeError(f"v1057 bundled pet feature missing: {token}")
    ...
    if "window.__NORTH_SHELL_BUILD__='1057'" not in shell_text or "delivery.js?v=1057" not in shell_text:
        raise RuntimeError("private shell is not v1057")
    if "<string>1057</string>" not in (bundle / "Info.plist").read_text(encoding="utf-8"):
        raise RuntimeError("PhoneWeb.bundle Info.plist is not v1057")

    project_text = (staging / "PhoneCompanionTest.xcodeproj/project.pbxproj").read_text(encoding="utf-8")
    if project_text.count("CURRENT_PROJECT_VERSION = 180;") != 12 or project_text.count("MARKETING_VERSION = 1.0.180;") != 12:
        raise RuntimeError("private iOS version is not consistently 1.0.180 (180)")
    webview_text = (staging / "PhoneCompanionTest/LocalPhoneWebView.swift").read_text(encoding="utf-8")
    if "__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.180 (180)'" not in webview_text:
        raise RuntimeError("private build marker is not 1.0.180 (180)")

...
test('private identifiers remain v1319 and iOS 393 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1319'/);
  assert.match(index, /app\.js\?v=1319&r=v1319-private-imsg-1/);
  ...
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 393;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.393;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1318 · 共同生活能改外观、能多选删除，让TA回真的点得动'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1318'/);
...
test('Mac guide identifies the current private v1319 iOS393 source', () => {

...
test('v1178 has a unique visible identity across every public entry and cache layer',()=>{
  assert.match(app,/__NORTH_SHELL_BUILD__!==\'1213\'/);
  assert.match(app,/APP_VER='v1213 · 请求诊断与简洁时间版'/);
  assert.match(shell,/__NORTH_SHELL_BUILD__='1213'/);
  assert.match(shell,/app\.js\?v=1213&r=v1213-couple-watch-trigger-1/);
  assert.match(index,/小手机\.html\?v=1213/);
  assert.match(repair,/小手机\.html\?v=1213/);
  assert.match(worker,/const BUILD='1213'/);
  assert.match(worker,/north-shell-v1213-couple-watch-trigger-1/);
  assert.match(hotfix,/sw\.js\?v=1213&r=v1213-couple-watch-trigger-1/);
...
test('the published backup path crosses the private bridge in bounded unicode-safe chunks',async()=>{
  ...
  const context=vm.createContext({
    ...
    privatePhoneAccountCall:async(action,payload)=>{
      calls.push(action);
      if(action==='account.backup.begin')return{transferId:'v1156-transfer',chunkCharacters:49152};
  ...
  const result=await context.upload(snapshot,{capturedAt:1156});

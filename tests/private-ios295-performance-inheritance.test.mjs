...
test('private identifiers remain v1279 and iOS 378 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1279'/);
  assert.match(index, /app\.js\?v=1279&r=v1279-private-backup-storage-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=339/);
  assert.match(app, /APP_VER='v1279 · 私人云备份提交超时修复'/);
  assert.match(overlay, /336-daily-file-backup/);
  assert.match(webview, /1\.0\.378 \(378\)/);
  assert.match(bridge, /private static let build = "1\.0\.378 \(378\)"/);
  assert.match(bridge, /static let contractVersion = 41/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 378;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.378;/g) || []).length, 12);
...
test('Mac guide identifies the current private v1279 iOS378 source', () => {

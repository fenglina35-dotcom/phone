/* Generated from the vendored MP4Box modules. Do not edit by hand. */
(function(global){
'use strict';
...
const TFHD_FLAG_DEFAULT_BASE_IS_MOOF = 131072;
const TRUN_FLAGS_FLAGS = 1024;
const TRUN_FLAGS_CTS_OFFSET = 2048;

//#endregion
//#region src/mp4boxbuffer.ts
var MP4BoxBuffer = class MP4BoxBuffer extends ArrayBuffer {
...
//#endregion
//#region src/DataStream.ts
let Endianness = /* @__PURE__ */ function(Endianness) {
	Endianness[Endianness["BIG_ENDIAN"] = 1] = "BIG_ENDIAN";
	Endianness[Endianness["LITTLE_ENDIAN"] = 2] = "LITTLE_ENDIAN";
	return Endianness;
}({});
var DataStream = class DataStream {
	static {
	...
	*/
	static arrayToNative(typedArray, endianness) {
		if (endianness === DataStream.ENDIANNESS) return typedArray;
	...
	*/
	static nativeToEndian(typedArray, littleEndian) {
		if (littleEndian && DataStream.ENDIANNESS === 2) return typedArray;
		else return this.flipArrayEndianness(typedArray);
	}
	/**
	...
	*/
	writeInt32Array(array, endianness) {
		...
		if (array instanceof Int32Array && this.byteOffset + this.position % array.BYTES_PER_ELEMENT === 0) {
			DataStream.memcpy(this._buffer, this.byteOffset + this.position, array.buffer, 0, array.byteLength);
	...
	writeType(type, value, struct) {
		...
		switch (parsedType) {
			...
			case "u16stringbe":
				...
				break;
			default: if (this.#isTupleType(parsedType)) {
				const [, ta] = parsedType;
		...
		for (; i < this.buffers.length; i++) {
			...
			if (ab.fileStart <= b.fileStart) {
				if (ab.fileStart === b.fileStart) if (ab.byteLength > b.byteLength) {
					...
					if (ab.fileStart + ab.byteLength <= b.fileStart) {} else ab = this.reduceBuffer(ab, 0, b.fileStart - ab.fileStart);
...
var SingleItemTypeReferenceBox = class extends Box {
	constructor(fourcc, size, box_name, hdr_size, start) {
		super(size);
...
var TrackReferenceTypeBox = class extends Box {
	constructor(fourcc, size, hdr_size, start) {
		super(size);
		this.hdr_size = hdr_size;
		this.start = start;
		this.type = fourcc;
...
function parseOneBox(stream, headerOnly, parentSize) {
	...
	} else size = stream.getEndPosition() - start;
...
//#endregion
//#region src/boxes/displays/parameterSetArray.ts
		...
		for (let i = 0; i < item_count; i++) {
			...
			switch (this.base_offset_size) {
				...
				case 8:
					...
					break;
				default: throw new Error("Error reading base offset size");
			}
			const extents = [];
			...
			for (let j = 0; j < extent_count; j++) {
				...
				if (this.version === 1 || this.version === 2) switch (this.index_size) {
					...
					case 8:
						...
						break;
					default: throw new Error("Error reading extent index");
				}
...
var irefBox = class irefBox extends FullBox {
	constructor(..._args) {
		super(..._args);
	...
	parse(stream) {
		...
		while (stream.getPosition() < this.start + this.size) {
			...
			if (ret.code === 1) {
				...
				if (!irefBox.allowed_types.includes(ret.type)) Log.warn("BoxParser", `Unknown item reference type: '${ret.type}'`);
...
//#endregion
//#region src/boxes/sampleentries/mett.ts
...
//#endregion
//#region src/boxes/sampleentries/metx.ts
...
//#endregion
//#region src/boxes/sampleentries/sampleentry.ts
/** @bundle box-codecs.js */
...
//#endregion
//#region src/boxes/sampleentries/sbtt.ts
...
//#endregion
//#region src/boxes/sampleentries/stpp.ts
...
//#endregion
//#region src/boxes/sampleentries/stxt.ts
...
//#endregion
//#region src/boxes/sampleentries/tx3g.ts
...
//#endregion
//#region src/boxes/sampleentries/wvtt.ts
...
var ISOFile = class ISOFile {
	...
	*/
	static writeInitializationSegment(ftyp, moov, total_duration, normalizeAudioSampleEntryTrackIds) {
		...
		ftyp.write(stream);
		const restoreCallbacks = ISOFile.normalizeAudioSampleEntriesForMSEFragmentedInit(moov.traks, normalizeAudioSampleEntryTrackIds);
		try {
	...
	initializeSegmentation(mode) {
		...
		if (mode === "per-track") return tracksToInitialize.map(({ id, user, trak }) => {
			...
			return {
				id,
				user,
				buffer: ISOFile.writeInitializationSegment(this.ftyp, moov, fragmentDuration, normalizeAudioSampleEntryTrackIds)
			};
		});
		const moov = new moovBox();
		moov.addBox(this.moov.mvhd);
	...
	*/
	...
	buildTrakSampleLists(trak) {
		...
		ISOFile.initSampleGroups(trak, void 0, sbgps, sgpds);
		...
		while (this.lastMoofIndex < this.moofs.length) {
			...
			if (box.type === "moof") {
				...
				for (let i = 0; i < moof.trafs.length; i++) {
					...
					if (traf.sbgps.length > 0) ISOFile.initSampleGroups(trak, traf, traf.sbgps, trak.mdia.minf.stbl.sgpds, traf.sgpds);

"""Create the private v1327 / iOS 397 Mac-source overlay package.
...
PREFIX = "SmallPhone_v1327_iOS397_Private/"
OUTPUT = ROOT.parent / "SmallPhone_v1327_iOS397_MacSource.zip"
...
def validate(files: dict[str, bytes]) -> None:
    ...
    for marker, label in [
        ...
        # ——— v1327 这一轮 ———
        ("function deliveryStructuredActionTags(value)", "v1327 外卖动作支持商品名内部嵌套中文方括号"),
        ("function deliveryIsolateStructuredActions(value)", "v1327 外卖动作与角色正文可靠分行"),
        ("function deliveryReplaceStructuredActions(value,replacer)", "v1327 通话路径同样完整消费嵌套外卖动作"),
        ("任何未完整消费的外卖控制标签都必须静默拦截", "v1327 畸形外卖标签不会显示成角色气泡"),
...
def main() -> None:
    ...
    with TemporaryDirectory(prefix="private1327-verify-") as folder:
        with ZipFile(OUTPUT) as archive:

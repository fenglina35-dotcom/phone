"""Create the private v1278 / iOS 377 Mac-source overlay package.
...
PREFIX = "SmallPhone_v1278_iOS377_Private/"
OUTPUT = ROOT.parent / "SmallPhone_v1278_iOS377_MacSource.zip"
...
def main() -> None:
    ...
    files["SOURCE_STATE.json"] = json.dumps({"sourceCommit":text(git("rev-parse","HEAD")).strip(),"upstreamBaseline":"69fb81f3747722091294c25c82af3d8409f8bbab","houseSource":"df56e66a4f24efed42e4c37ef7d1908deb95df03","privateWeb":"v1278","privateIOS":"1.0.377 (377)","bridge":40,"macBuildVerified":False,"realIPhoneVerified":False,"kind":"Mac-source-not-IPA","preserved":["all-public-v1275-features","all-private-v1278-features","private-backup-4mib-object-chunks","private-backup-small-manifest-commit","private-backup-checksum-restore","private-backup-legacy-fallback","private-backup-single-upload-progress","web-cloud-existing-row-update","web-cloud-persistent-progress","private-backup-commit-timeout-720s","online-memory-recall-controls","offline-memory-scroll-restore","cozy-private-native-bridge","diagnostic-copy-freshness","native-probe-lifecycle","house052-complete-assets","first-person-head-tracking","isolated-mobile-shader-warmup","warm-gpu-resource-release","female-standing-pose"]},ensure_ascii=False,indent=2).encode("utf-8")
    ...
    with TemporaryDirectory(prefix="private1278-verify-") as folder:
        with ZipFile(OUTPUT) as archive:

...
test('private v1279 inherits the current web theater without dropping its private shell',()=>{
  ...
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1279[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1279[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1279&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1279 and iOS 1.0.378 (378)',()=>{
  assert.match(privateApp,/const APP_VER='v1279 · 私人云备份提交超时修复'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.378 \(378\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 378;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.378;/g)||[]).length>=12);
});

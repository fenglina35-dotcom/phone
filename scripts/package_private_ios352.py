...
def validate(web,private):
 ...
 for s in [app,papp]:
  assert "APP_VER='v1229" in s
  for key in ['explicitVoiceReplyRequest','requestedVoiceNeedsFix','voiceReplyBlocked','forceRequestedVoiceReply','finalizeVoiceReply','roleVoiceFormatRepair','voiceReplyCanRepairCandidate','这条明确请求优先于“平时不主动发语音”的频率设置']:assert key in s,key
 ...
 for s in [html,phtml]:assert "__NORTH_SHELL_BUILD__='1229'" in s and 'app.js?v=1229' in s
 assert "BUILD='1229'" in text(web['sw.js'])
 ...
 assert project.count('CURRENT_PROJECT_VERSION = 352;')==12 and project.count('MARKETING_VERSION = 1.0.352;')==12
 ...
 assert 'daily-event-ledger.js?v=1229' in html and 'daily-event-ledger.js?v=1229' in phtml
 for s in [app,papp]:
  assert 'offlineStripMoodTags' in s and "['events','日常事件簿'" in s
 assert '日常事件簿' in text(private[BUNDLE+'daily-event-ledger.js'])
 for s in [app,papp]:
  assert "events:'notebook'" in s and "grudge:'notebook'" in s
  assert '自动记录日常事件' in text(private[BUNDLE+'daily-event-ledger.js'])
 assert 'v1229' in text(private[GUIDE]) and '日常事件簿' in text(private[GUIDE])
 ...
 sync=text(private['PhoneCompanionTest/CompanionSyncView.swift'])
 for key in ['.id(reportRefreshID)', 'performTodayUsageReadWithTimeout', '复制屏幕同步诊断', '"requestedAt": request.requestedAt']:
  assert key in sync,key
...
def main():
 ...
 for scope,files,name,prefix in [('私人',private,'小手机_v1229_私人版_iOS352_屏幕同步修复候选_Mac待编译源码包.zip','SmallPhone_v1229_iOS352_MacSource/')]:
  files['SOURCE_COMMIT.txt']=f'commit={head}\nversion=v1229\nscope={scope}\nios=1.0.352 (352)\nbridge=37\nprivate-cloud-avatar-migration=VERIFIED-2026-09-09\nmac-build-verified=no\nreal-iphone-verified=no\nkind=source-only-not-IPA\nweb-deployment=verify-separately\ndaily-event-ledger=included\nexplicit-voice-request-repair=included\nnative-screen-time-request-repair=included\nreal-screen-time-sync-verified=no\n'.encode()

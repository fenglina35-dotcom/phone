...
test('thermal pressure and repeated termination stop WebKit self-reload loops', () => {
  ...
  assert.match(webView, /webContentTerminationTimes\.v17\.build308/);
...
test('both private entry files and every iOS target carry build 308', () => {
  ...
  assert.match(bridge, /1\.0\.308 \(308\)/);
  assert.equal(
    (project.match(/CURRENT_PROJECT_VERSION = 308;/g) || []).length,
    12
  );
  assert.equal(
    (project.match(/MARKETING_VERSION = 1\.0\.308;/g) || []).length,
    12
  );
});

...
test('private build and bundled recovery page advance together', () => {
  ...
  assert.match(webView, /smallPhone\.webContentTerminationTimes\.v25\.build323/);
  assert.match(privateApp, /APP_VER='v1202 · 像素少女照顾版'/);
  assert.equal(privateAlias, privateIndex);
  assert.match(privateIndex, /window\.__NORTH_SHELL_BUILD__='1202'/);
  assert.match(privateIndex, /app\.js\?v=1202/);
  assert.match(privateIndex, /private-runtime-diagnostics\.js\?v=323/);
  assert.match(privateRepair, /index\.html\?repair=1&v=1202/);
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!==\'1202\'/);
});

...
test('private build and bundled recovery page advance together', () => {
  ...
  assert.match(privateApp, /APP_VER='v1128 · 备份与线下回复修复版'/);
  assert.equal(privateAlias, privateIndex);
  assert.match(privateIndex, /window\.__NORTH_SHELL_BUILD__='1128'/);
  assert.match(privateIndex, /app\.js\?v=1128/);
  assert.match(privateIndex, /private-runtime-diagnostics\.js\?v=260/);
  assert.match(privateRepair, /index\.html\?repair=1&v=1128/);
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!==\'1128\'/);
});

...
test('private iOS 291 overlay owns the disable marker and cancels automatic scheduling', async () => {
...
test('manual backup and both restore actions remain free of the automatic-disable guard', () => {
  for (const name of [
    ...
    assert.doesNotMatch(source, /291-scroll-camera-smart-home-v1/);
  }
  assert.doesNotMatch(
    overlay,
...
test('native recovery UI stays outside WebKit and carries the private 291 identity', () => {
  ...
  assert.match(webView, /smallPhone\.webContentTerminationTimes\.v5\.build291/);
  assert.match(bridge, /private static let build = "1\.0\.291 \(291\)"/);

...
test('v1189 private identifiers retain the performance chain while public stays v1184', () => {
  ...
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1203'/);
  assert.match(index, /app\.js\?v=1203&r=v1203-couple-watch-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=324/);
  assert.match(app, /APP_VER='v1203 · 像素少女照顾版'/);
  assert.match(overlay, /324-pixel-home-2/);
  assert.match(webview, /1\.0\.324 \(324\)/);
  assert.match(bridge, /private static let build = "1\.0\.324 \(324\)"/);
  assert.match(bridge, /static let contractVersion = 35/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 324;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.324;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1203 · 像素少女照顾版'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1203'/);
...
test('current Mac guides identify private v1203 and web not deployed', () => {
  const install = fs.readFileSync(
    path.join(project, '第三百二十四次安装_v1203_像素少女_请先读.md'),
    'utf8',
  );
  const mac = fs.readFileSync(path.join(project, '请在Mac编译前先读.md'), 'utf8');
  for (const guide of [install, mac]) {
    assert.match(guide, /v1203/);
    assert.match(guide, /网页.*推送/);
    assert.match(guide, /1\.0\.324 \(324\)/);
  ...
  assert.match(mac, /^# v1203 .*私人源码候选/);
  assert.match(mac, /网页.*推送/);
  assert.match(install, /网页源码与私人内置网页 v1203/);

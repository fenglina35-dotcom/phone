...
test('private iOS 288 overlay owns the disable marker and cancels automatic scheduling', async () => {
  const { context, calls } = makeAutoBackupHarness();
  assert.equal(
    context.__SMALL_PHONE_PRIVATE_RUNTIME__,
    '288-delivery-unknown-brand-v1'
  );
...
test('manual backup and both restore actions remain free of the automatic-disable guard', () => {
  for (const name of [
    ...
    assert.doesNotMatch(source, /288-delivery-unknown-brand-v1/);
  }
  assert.doesNotMatch(
    overlay,
...
test('native recovery UI stays outside WebKit and carries the private 288 identity', () => {
  ...
  assert.match(webView, /smallPhone\.webContentTerminationTimes\.v5\.build288/);
  assert.match(bridge, /private static let build = "1\.0\.288 \(288\)"/);

"""Create the private v1275 / iOS 374 Mac-source overlay package.
...
PREFIX = "SmallPhone_v1275_iOS374_Private/"
OUTPUT = ROOT.parent / "SmallPhone_v1275_iOS374_MacSource.zip"
...
def main() -> None:
    ...
    files["SOURCE_STATE.json"] = json.dumps({"sourceCommit":text(git("rev-parse","HEAD")).strip(),"upstreamBaseline":"a3a95087cd13989a9e02d8d69b5a4273bb201ee3","houseSource":"df56e66a4f24efed42e4c37ef7d1908deb95df03","privateWeb":"v1275","privateIOS":"1.0.374 (374)","bridge":38,"macBuildVerified":False,"realIPhoneVerified":False,"kind":"Mac-source-not-IPA","preserved":["all-public-v1274-features","all-private-v1274-features","private-backup-commit-timeout-720s","online-memory-recall-controls","offline-memory-scroll-restore","cozy-private-native-bridge","diagnostic-copy-freshness","native-probe-lifecycle","house052-complete-assets","first-person-head-tracking","isolated-mobile-shader-warmup","warm-gpu-resource-release","female-standing-pose"]},ensure_ascii=False,indent=2).encode("utf-8")
    ...
    with TemporaryDirectory(prefix="private1275-verify-") as folder:
        with ZipFile(OUTPUT) as archive:

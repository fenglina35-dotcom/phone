...
test('private artifact identity advances independently to v1278 and iOS 1.0.377 (377)',()=>{
  assert.match(privateApp,/const APP_VER='v1278 · 私人云备份分块存储修复'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.377 \(377\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 377;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.377;/g)||[]).length>=12);
});

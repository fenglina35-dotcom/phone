...
test('private identifiers remain v1275 and iOS 374 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1275'/);
  assert.match(index, /app\.js\?v=1275&r=v1275-private-sync-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=339/);
  assert.match(app, /APP_VER='v1275 · 私人版云备份与记忆设置修复'/);
  assert.match(overlay, /336-daily-file-backup/);
  assert.match(webview, /1\.0\.374 \(374\)/);
  assert.match(bridge, /private static let build = "1\.0\.374 \(374\)"/);
  assert.match(bridge, /static let contractVersion = 38/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 374;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.374;/g) || []).length, 12);
...
test('Mac guide identifies the current private v1275 iOS374 source', () => {

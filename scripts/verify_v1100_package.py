...
package = root / "delivery-v1100-private224-final" / "SmallPhone_v1100_RolePhotoFrequencyCards_iOS224_MacReady.zip"
...
with ZipFile(package) as archive:
    ...
    guide = next((name for name in names if name.endswith("第二百二十四次安装_v1100_图片频率图文朋友圈_请先读.md")), "")
    if not guide:
        raise RuntimeError("missing v1100 install readme")
    prefix = guide.split("第二百二十四次安装", 1)[0] + "PhoneCompanionTest/PhoneWeb.bundle/"
    ...
    for token in [
        "const APP_VER='v1100 · 图片频率与图文朋友圈版'",
        "if(c.photoFreqEnabled==null)c.photoFreqEnabled=false",
        "function roleSocialCardPlan",
        "function roleSocialMedia",
        "添加图文照片",
    ]:
        if token not in app:
            raise RuntimeError(f"bundled role-photo fix missing: {token}")
    if "window.__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.224 (224)'" not in webview:
        raise RuntimeError("bundled private build marker mismatch")
    if project.count("CURRENT_PROJECT_VERSION = 224;") != 12 or project.count("MARKETING_VERSION = 1.0.224;") != 12:
...
print("v1100 package structure, byte identity, version and role-photo checks passed")

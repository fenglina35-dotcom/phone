...
test('private identifiers advance to v1236 and iOS 355 while public stays v1236', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1236'/);
  assert.match(index, /app\.js\?v=1236&r=v1236-backup-auto-download-1/);
  ...
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1236'/);
...
test('Mac guide distinguishes v1236 source sync from the unchanged iOS355 package', () => {
  ...
  assert.match(mac, /^# v1236 .*仅同步内置网页源码/);

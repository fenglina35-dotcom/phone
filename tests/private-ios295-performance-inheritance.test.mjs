...
test('private identifiers advance to v1269 and iOS 369 while public stays v1269', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1269'/);
  assert.match(index, /app\.js\?v=1269&r=v1269-private-sync-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=336/);
  assert.match(app, /APP_VER='v1269 · 抖音发作品与输入栏'/);
  assert.match(overlay, /336-daily-file-backup/);
  assert.match(webview, /1\.0\.369 \(369\)/);
  assert.match(bridge, /private static let build = "1\.0\.369 \(369\)"/);
  assert.match(bridge, /static let contractVersion = 38/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 369;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.369;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1269 · 抖音发作品与输入栏'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1269'/);
...
test('Mac guide identifies the current private v1269 iOS369 source', () => {

...
test('v1180 web identity and cache markers are aligned', () => {
  ...
  assert.match(sw, /v1180-summary-call-offline-1/);
  assert.match(hotfix, /v1180-summary-call-offline-1/);
  assert.match(index, /小手机\.html\?v=1180/);
  assert.match(repair, /小手机\.html\?v=1180/);
});

test('v1180 publishes shared cohab memory and X comment controls', () => {
  const app = read('app.js');

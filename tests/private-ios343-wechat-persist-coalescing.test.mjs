...
test('private identity advances to v1237 and iOS 355 while public remains v1237', () => {
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!=='1237'/);
  assert.match(privateApp, /APP_VER='v1237 · 大备份分段读写'/);
  for (const html of [privateIndex, privateAlias]) {
    assert.match(html, /window\.__NORTH_SHELL_BUILD__='1237'/);
    assert.match(html, /app\.js\?v=1237&r=v1237-backup-streaming-1/);
    assert.match(html, /private-runtime-diagnostics\.js\?v=335/);
  }
  assert.match(privateRepair, /index\.html\?repair=1&v=1237/);
  ...
  assert.match(publicIndex, /window\.__NORTH_SHELL_BUILD__='1237'/);

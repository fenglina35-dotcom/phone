...
(function(){
  ...
  var PREF_CONFIG={
    ...
    kfc:{title:'KFC 偏好',icon:'🍗',eyebrow:'KFC',fields:[['favorites','喜欢的单品','例如：香辣鸡腿堡、原味鸡、蛋挞；不使用套餐'],['sides','小食偏好','例如：薯条、鸡米花，不要玉米杯'],['drinks','饮料偏好','例如：无糖可乐、热牛奶'],['custom','份量和定制','例如：两块原味鸡、汉堡不要酱'],['avoid','不喜欢、过敏或忌口','例如：不吃辣；不要花生碎']]}
  };
  ...
  async function payOrder(order){
    ...
    order.status=text(data.status,40)||'pending_payment';order.paymentMethod=text(data.paymentMethod,40);order.payUrl=safePayUrl(data.payUrl);order.payQrDataUrl=safePayQr(data.payQrDataUrl);order.etaText=safeEtaText(data.etaText)||order.etaText||'';order.pendingReason=text(data.reason,180);order.updatedAt=Date.now();
  ...
  function safeEtaText(value){var m=text(value,80).match(/(\d{1,2}):([0-5]\d)\s*[-–—至~～]\s*(\d{1,2}):([0-5]\d)/);if(!m||+m[1]>23||+m[3]>23)return'';return String(+m[1]).padStart(2,'0')+':'+m[2]+'–'+String(+m[3]).padStart(2,'0')+':'+m[4]+'送达';}
  ...
  function notifyStatus(order,before){
    ...
    var exactEta=safeEtaText(order.etaText),fact='[真实外卖订单状态更新]\n你亲自创建的真实外卖订单现在由平台确认处于「'+statusText(order.status)+'」。商家：'+(order.merchant||'')+'；餐品：'+(order.items||[]).map(function(x){return x.name;}).join('、')+'；实付/待付金额：¥'+num(order.total).toFixed(2)+(exactEta?'；平台预计送达：'+exactEta:'')+'。这是平台刚返回的事实。请按你自己的完整人设、当前关系和说话习惯，自主决定怎样提醒'+S.me.name+'；不要照抄提示，不要添加骑手位置或其他未提供事实。若只是中间小变化且你本人觉得不必打扰，也可以不发。';

...
DELIVERY = ROOT / "delivery-v1038"
PACKAGE_NAME = "SmallPhone_v1038_RealDeliveryReply_iOS158_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百五十八次安装_v1038_真实外卖回复闭环_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1038":
...
if "APP_VER='v1038 · 真实外卖回复闭环';" not in app_text:
    raise RuntimeError("bundled web version is not v1038")
...
if project_text.count("CURRENT_PROJECT_VERSION = 158;") != 12:
    raise RuntimeError("private iOS build number is not consistently 158")
if project_text.count("MARKETING_VERSION = 1.0.158;") != 12:
    raise RuntimeError("private iOS marketing version is not consistently 1.0.158")

...
test('private iOS 312 overlay owns the disable marker and cancels automatic scheduling', async () => {
...
test('manual backup and both restore actions remain free of the automatic-disable guard', () => {
  for (const name of [
    ...
    assert.doesNotMatch(source, /312-private-v1186-cohab-keyboard-isolation-1/);
  }
  assert.doesNotMatch(
    overlay,
...
test('native recovery UI stays outside WebKit and carries the private 312 identity', () => {

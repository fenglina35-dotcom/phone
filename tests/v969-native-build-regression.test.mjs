...
test('v1017 web and private iOS 1.0.138 keep explicit build identities', () => {
  assert.match(app, /APP_VER='v1017 · 小事簿后台入聊与气泡配色修复'/);
  assert.match(app, /sw\.js\?v=1017&r=v1017-notebook-inbox-bubble-repair-1/);
  assert.match(shell, /north-shell-v1017/);
  assert.match(bundleInfo, /<string>1017<\/string>/);
  assert.match(localWebView, /1\.0\.138 \(138\)/);

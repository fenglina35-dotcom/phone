...
test('private build and bundled recovery page advance together', () => {
  ...
  assert.match(webView, /smallPhone\.webContentTerminationTimes\.v25\.build317/);
  assert.match(privateApp, /APP_VER='v1195 · 情侣空间触发计数版'/);
  assert.equal(privateAlias, privateIndex);
  assert.match(privateIndex, /window\.__NORTH_SHELL_BUILD__='1195'/);
  assert.match(privateIndex, /app\.js\?v=1195/);
  assert.match(privateIndex, /private-runtime-diagnostics\.js\?v=317/);
  assert.match(privateRepair, /index\.html\?repair=1&v=1195/);
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!==\'1195\'/);
});

...
test('private identifiers advance to v1217 and iOS 342 while public stays v1216', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1217'/);
  assert.match(index, /app\.js\?v=1217&r=v1217-private-homekit-air-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=333/);
  assert.match(app, /APP_VER='v1217 · 私人智能空调版'/);
  assert.match(overlay, /333-background-lane/);
  assert.match(webview, /1\.0\.342 \(342\)/);
  assert.match(bridge, /private static let build = "1\.0\.342 \(342\)"/);
  assert.match(bridge, /static let contractVersion = 37/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 342;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.342;/g) || []).length, 12);
...
test('current Mac guide advances to private v1217 and iOS 342 while the historical iOS 327 record remains historical', () => {

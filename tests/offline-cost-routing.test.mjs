...
assert.match(source, /routeIndex=roleChatRouteIndex\(c\),auditOpt=\(opt,stage\)=>Object\.assign\(\{\},opt,\{routeIndex,allowSessionModel:true,roleInterceptAudit:_offAudit,roleInterceptStage:stage\}\)/);
assert.match(source, /rawReply=await offlineReplyChatRequest\(req,auditOpt\(\{aux:false,max:replyMax,temp:\.75,unfilteredOutput:_rawOutput\},'单次约会主候选'\)\);let r=_rawOutput\?modelUnfilteredText\(rawReply\):roleVisibleEnvelopeText\(rawReply\)/);

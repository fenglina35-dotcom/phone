...
DELIVERY = ROOT / "delivery-v1070-private194-final"
PACKAGE_NAME = "SmallPhone_v1070_PrivateWebContentRecovery_iOS194_MacReady"
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百九十四次安装_v1070_私人App_WebContent恢复与内存边界_请先读.md"
...
with tempfile.TemporaryDirectory(prefix="smallphone-v1070-ios194-", dir=ROOT) as temp:
    ...
    for token in [
        "APP_VER='v1070 · 私人App WebContent恢复与内存边界版';",
    ...
    if "window.__NORTH_SHELL_BUILD__='1070'" not in shell_text:
        raise RuntimeError("private shell is not v1070")
    if "<string>1070</string>" not in (bundle / "Info.plist").read_text(encoding="utf-8"):
        raise RuntimeError("PhoneWeb.bundle Info.plist is not v1070")

    project_text = (staging / "PhoneCompanionTest.xcodeproj/project.pbxproj").read_text(encoding="utf-8")
    if project_text.count("CURRENT_PROJECT_VERSION = 194;") != 12:
        raise RuntimeError("private build number is not consistently 194")
    if project_text.count("MARKETING_VERSION = 1.0.194;") != 12:
        raise RuntimeError("private marketing version is not consistently 1.0.194")

    webview_text = (staging / "PhoneCompanionTest/LocalPhoneWebView.swift").read_text(encoding="utf-8")
    for token in [
        "__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.194 (194)'",

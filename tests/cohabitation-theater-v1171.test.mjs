...
test('private v1303 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1302[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1302&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1303[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1303[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1303&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1303 and iOS 1.0.385 (385)',()=>{
  assert.match(privateApp,/const APP_VER='v1303 · 抖音发作品修复、文字特效、隐形墨水刮开（私人）'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.385 \(385\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 385;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.385;/g)||[]).length>=12);
});

...
assert.match(app,/if\(!_rawOutput&&_realDeliveryCommandTurn&&!_realDeliveryTag\)/,'a command turn must filter unsafe premature result text');
...
assert.match(app,/if\(!_rawOutput&&_callRealDeliveryCommandTurn\)content=_callDeliveryPrelude/,'call ordering must retain only the current-model role prelude');

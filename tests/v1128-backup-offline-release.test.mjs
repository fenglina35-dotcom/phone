...
test('v1145 has a unique visible identity across every public entry and cache layer',()=>{
  assert.match(app,/__NORTH_SHELL_BUILD__!==\'1145\'/);
  assert.match(app,/APP_VER='v1145 · 共同生活入口兼容修复版'/);
  assert.match(shell,/__NORTH_SHELL_BUILD__='1145'/);
  assert.match(shell,/app\.js\?v=1145&r=v1145-cohab-entry-webkit-1/);
  assert.match(index,/小手机\.html\?v=1145/);
  assert.match(repair,/小手机\.html\?v=1145/);
  assert.match(worker,/const BUILD='1145'/);
  assert.match(worker,/north-shell-v1145-cohab-entry-webkit-1/);
  assert.match(hotfix,/sw\.js\?v=1145&r=v1145-cohab-entry-webkit-1/);
...
test('the published backup path crosses the private bridge in bounded unicode-safe chunks',async()=>{
  ...
  const context=vm.createContext({
    ...
    privatePhoneAccountCall:async(action,payload)=>{
      calls.push(action);
      if(action==='account.backup.begin')return{transferId:'v1145-transfer',chunkCharacters:49152};
  ...
  const result=await context.upload(snapshot,{capturedAt:1145});

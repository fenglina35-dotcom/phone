...
const ctx={
  ...
  chatAPI:async()=>'{"offerId":"offer-1","quantity":1}',
...
ctx.fetch=async(_url,init)=>{
  ...
  if(body.action==='pay_order')return {ok:true,status:200,json:async()=>({ok:true,data:{status:'paid',paymentMethod:'wechat'}})};
...
assert.equal(ctx.S.food.orders[0].status,'paid','connector paid receipt is preserved');
assert.equal(role.deliveryWallet.balance,80,'role allowance is debited only after paid receipt');
...
assert.match(notices[0].note,/当前真实状态：已付款/);

...
def validate(web,private):
 ...
 for s in [app,papp]:
  assert "APP_VER='v1226" in s
  for key in ['explicitVoiceReplyRequest','requestedVoiceNeedsFix','voiceReplyBlocked','forceRequestedVoiceReply','这条明确请求优先于“平时不主动发语音”的频率设置']:assert key in s,key
 ...
 for s in [html,phtml]:assert "__NORTH_SHELL_BUILD__='1226'" in s and 'app.js?v=1226' in s
 assert "BUILD='1226'" in text(web['sw.js'])
 ...
 assert project.count('CURRENT_PROJECT_VERSION = 349;')==12 and project.count('MARKETING_VERSION = 1.0.349;')==12
 ...
 assert 'daily-event-ledger.js?v=1226' in html and 'daily-event-ledger.js?v=1226' in phtml
 for s in [app,papp]:
...
def main():
 ...
 for scope,files,name,prefix in [('私人',private,'小手机_v1226_私人版_iOS349_明确语音请求修复_Mac待编译源码包.zip','SmallPhone_v1226_iOS349_MacSource/')]:

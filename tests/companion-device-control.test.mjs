...
test('prototype data is clearly non-device data and version is aligned', () => {
  ...
  assert.match(app, /const APP_VER='v1320 · 共同生活只画最近300条、气泡改磨砂、我的气泡回浅黄'/);
});
...
test('all-data read cannot let the role speak before the same native session is complete', () => {
  ...
  const callQueue = app.indexOf('const _nativeCallInspectionQueued=!_rawOutput&&!_screenShareEvent&&!_inspectionCompletion&&!_videoVision&&maybeSpyIntent');

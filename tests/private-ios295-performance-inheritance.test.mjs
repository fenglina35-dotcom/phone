...
test('private identifiers remain v1321 and iOS 394 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1321'/);
  assert.match(index, /app\.js\?v=1321&r=v1321-private-imsg-1/);
  ...
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 394;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.394;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1320 · 共同生活只画最近300条、气泡改磨砂、我的气泡回浅黄'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1320'/);
...
test('Mac guide identifies the current private v1321 iOS394 source', () => {

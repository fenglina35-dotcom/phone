...
DELIVERY = ROOT / "delivery-v1036"
PACKAGE_NAME = "SmallPhone_v1036_RealMilkTea_WeChatDiscover_iOS156_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百五十六次安装_v1036_真实奶茶与微信发现页合并版_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1036":
...
if "APP_VER='v1036 · 真实奶茶与微信发现页合并版';" not in app_text:
    raise RuntimeError("bundled web version is not v1036")
if "淘宝闪购真实外卖" not in app_text and "真实奶茶" not in app_text:
    raise RuntimeError("real milk tea ordering source is missing")
if "附近的人" not in app_text or "wxNearbyRefresh" not in app_text:
    raise RuntimeError("WeChat discover or nearby people source is missing")
project_text = (STAGING / "PhoneCompanionTest.xcodeproj" / "project.pbxproj").read_text(
    encoding="utf-8"
)
if project_text.count("CURRENT_PROJECT_VERSION = 156;") != 12:
    raise RuntimeError("private iOS build number is not consistently 156")
if project_text.count("MARKETING_VERSION = 1.0.156;") != 12:
    raise RuntimeError("private iOS marketing version is not consistently 1.0.156")

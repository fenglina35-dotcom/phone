...
package = root / "delivery-v1090-private215-final" / "SmallPhone_v1090_NaturalDeliveryCommitment_iOS215_MacReady.zip"
...
with ZipFile(package) as archive:
    ...
    guide = next((name for name in names if name.endswith("第二百一十五次安装_v1090_自然外卖承诺执行修复_请先读.md")), "")
    if not guide:
        raise RuntimeError("missing v1090 install readme")
    prefix = guide.split("第二百一十五次安装", 1)[0] + "PhoneCompanionTest/PhoneWeb.bundle/"
    ...
    for token in [
        "const APP_VER='v1090 · 自然外卖承诺执行修复版'",
    ...
    for token in [
        "function delegatedChoiceIntent",
        "function roleCommittedToDelivery",
        "function missingActionRetryPrompt",
        "window.deliveryReportActionRepairFailure=reportActionRepairFailure",
    ]:
        if token not in delivery:
            raise RuntimeError(f"bundled delivery fix missing: {token}")
    if "window.__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.215 (215)'" not in webview:
        raise RuntimeError("bundled private build marker mismatch")
    if project.count("CURRENT_PROJECT_VERSION = 215;") != 12 or project.count("MARKETING_VERSION = 1.0.215;") != 12:
        raise RuntimeError("bundled iOS version mismatch")
    for required in ["app.js", "delivery.js", "index.html", "小手机.html"]:
        if required not in archived_bundle:
            raise RuntimeError(f"required bundle file missing: {required}")
    for ignored_required in ["vendor/qr/jsQR.js", "vendor/qr/qrcode.js", "wechat-me.css", "wechat-me.js"]:
        if ignored_required not in archived_bundle:
            raise RuntimeError(f"gitignored runtime file missing: {ignored_required}")
...
print("v1090 package structure, byte identity, version and protected-route checks passed")

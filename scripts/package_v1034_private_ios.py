...
DELIVERY = ROOT / "delivery-v1034"
PACKAGE_NAME = "SmallPhone_v1034_WeChatContactsFixed_iOS154_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百五十四次安装_v1034_微信通讯录固定导航修复_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1034":
...
if "APP_VER='v1034 · 微信通讯录固定导航修复';" not in app_text:
    raise RuntimeError("bundled web version is not v1034")
project_text = (STAGING / "PhoneCompanionTest.xcodeproj" / "project.pbxproj").read_text(
    encoding="utf-8"
)
if project_text.count("CURRENT_PROJECT_VERSION = 154;") != 12:
    raise RuntimeError("private iOS build number is not consistently 154")
if project_text.count("MARKETING_VERSION = 1.0.154;") != 12:
    raise RuntimeError("private iOS marketing version is not consistently 1.0.154")

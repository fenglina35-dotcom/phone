...
test('private identifiers remain v1323 and iOS 394 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1323'/);
  assert.match(index, /app\.js\?v=1323&r=v1323-private-imsg-1/);
  ...
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 395;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.395;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1322 · 气泡即时配色与角色偏好、情侣任务和收款回执'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1322'/);
...
test('Mac guide identifies the current private v1323 iOS395 source', () => {

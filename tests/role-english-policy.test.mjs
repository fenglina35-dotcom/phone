...
for(const path of paths){
 ...
 function harness(){const ctx={Error,String,JSON,Array,Object,chatAPI:async()=>{throw Error('unexpected request');},joinAIContinuation:(a,b)=>a+b};vm.createContext(ctx);vm.runInContext(['roleReplyEnglishOnly','roleReplyDropEnglishNarration','roleReplyAssertLanguage','roleThoughtFormatPrompt','chatResultText'].map(n=>extract(src,n)).join('\n'),ctx);return ctx;}

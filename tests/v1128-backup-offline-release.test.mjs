...
test('v1128 has a unique visible identity across every public entry and cache layer',()=>{
  assert.match(app,/__NORTH_SHELL_BUILD__!==\'1128\'/);
  assert.match(app,/APP_VER='v1128 · 备份与线下回复修复版'/);
  assert.match(shell,/__NORTH_SHELL_BUILD__='1128'/);
  assert.match(shell,/app\.js\?v=1128&r=v1128-backup-offline-failure-evidence-1/);
  assert.match(index,/小手机\.html\?v=1128/);
  assert.match(repair,/小手机\.html\?v=1128/);
  assert.match(worker,/const BUILD='1128'/);
  assert.match(worker,/north-shell-v1128-backup-offline-failure-evidence-1/);
  assert.match(hotfix,/sw\.js\?v=1128&r=v1128-backup-offline-failure-evidence-1/);
...
test('the published backup path crosses the private bridge in bounded unicode-safe chunks',async()=>{
  ...
  const context=vm.createContext({
    ...
    privatePhoneAccountCall:async(action,payload)=>{
      calls.push(action);
      if(action==='account.backup.begin')return{transferId:'v1128-transfer',chunkCharacters:49152};
  ...
  const result=await context.upload(snapshot,{capturedAt:1128});

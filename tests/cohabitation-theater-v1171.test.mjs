...
test('private v1313 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1312[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1312&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1313[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1313[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1313&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1313 and iOS 1.0.390 (390)',()=>{
  assert.match(privateApp,/const APP_VER='v1313 · 短信效果听她的、多选删除、一键清空、记仇本真的记（私人）'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.390 \(390\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 390;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.390;/g)||[]).length>=12);
});

...
test('v1184 web keeps private 1.0.311 compatibility',()=>{
  assert.match(app,/APP_VER='v1184 · iPhone稳定、剧场顺序与键盘修复版'/);
  assert.match(html,/__NORTH_SHELL_BUILD__='1184'/);
  assert.match(sw,/BUILD='1184'/);
  assert.match(plist,/<string>1184<\/string>/);
  assert.equal((project.match(/CURRENT_PROJECT_VERSION = 311;/g)||[]).length,12);
  assert.equal((project.match(/MARKETING_VERSION = 1\.0\.311;/g)||[]).length,12);
  assert.match(native,/1\.0\.311 \(311\)/);
});

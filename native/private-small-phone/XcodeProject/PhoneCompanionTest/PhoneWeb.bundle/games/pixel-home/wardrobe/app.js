...
function renderRaw(g,c,images,s,{frame='open',time=0,animate=false,only=null}={}){
 ...
 if(!only){
  ...
  if(!cache||cache.key!==key){const b=c.items[s.dress].base,layer=new OffscreenCanvas(1024,1536),bg=layer.getContext('2d');bg.imageSmoothingEnabled=false;drawPiece(bg,b,images);
...
function drawLegLayer(g,layer,k,start=0,width=1){
 ...
 let middle=layer;
 if(width!==1){middle=new OffscreenCanvas(1024,1536);const m=middle.getContext('2d');m.imageSmoothingEnabled=false;
...
function breathLayer(layer,n){
 const out=new OffscreenCanvas(1024,1536),g=out.getContext('2d');g.imageSmoothingEnabled=false;
...
const categories={outfits:'套装',dress:'裙装',shoes:'鞋袜',hairs:'发型',faces:'五官',accessories:'发饰'};
...
function syncBody(){const bp=bodyParams(state);for(const k of ['size','legs','legWidth']){$('#body-'+k).value=bp[k];$('#body-'+k+'-value').textContent=bp[k]+'%';}}
...
function resize(){const stage=$('#stage');canvas.style.height=Math.round(stage.clientHeight*(zoom?1:previewScale/100))+'px';canvas.classList.toggle('editing',$('#adjust').open);const r=canvas.getBoundingClientRect(),d=Math.min(devicePixelRatio||1,2);canvas.width=Math.round(r.width*d);canvas.height=Math.round(r.height*d);const crop=zoom?[210,65,610,740]:[210,45,610,1440];const z=Math.min(r.width/crop[2],(r.height-20)/crop[3]);coords={d,z,x:(r.width-z*crop[2])/2-crop[0]*z,y:(r.height-20-z*crop[3])/2-crop[1]*z};}
...
$('#export').onclick=()=>{save();download(new Blob([JSON.stringify({schema:'rose-wardrobe',version:1,savedAt:new Date().toISOString(),state,sourceParameters:catalog.sources,assetHashes:catalog.files},null,2)],{type:'application/json'}),'我的衣柜-配置-P82.json')};
$('#import').onchange=async e=>{try{const f=e.target.files[0];if(!f)return;if(f.size>5e6)throw Error();const d=JSON.parse(await f.text());if(d.schema!=='rose-wardrobe'||!valid(d.state,catalog)||JSON.stringify(d.assetHashes)!==JSON.stringify(catalog.files))throw Error();remember();state=d.state;targets();rack();save()}catch{notify('配置不匹配，原搭配已保留')}e.target.value=''};

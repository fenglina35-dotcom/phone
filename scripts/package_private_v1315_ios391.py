"""Create the private v1315 / iOS 391 Mac-source overlay package.
...
PREFIX = "SmallPhone_v1315_iOS391_Private/"
OUTPUT = ROOT.parent / "SmallPhone_v1315_iOS391_MacSource.zip"
...
def validate(files: dict[str, bytes]) -> None:
    ...
    for marker, label in [
        ...
        # ——— v1315 这一轮 ———
        ("s+='\\n- 聊天背景：'+S.me.name+'让你把某张照片换成你们的聊天背景", "v1315 换背景说明单独发，不再挂在表情包的 if 里"),
        ("只在嘴上说「换好了」是没有用的", "v1315 光说没用，写死在提示词里"),
        ("function wechatBgRequest(text)", "v1315 认出她在要换背景"),
        ("function wechatApplyBgRequest(c,id)", "v1315 他忘了写标签就替他换上"),
        ("if(_wantBg&&!_bgApplied)wechatApplyBgRequest(c,id);", "v1315 收尾兜底"),
        ("const OFF_THEME_DEF=", "v1315 线下配色"),
        ("function offStageAttrs(c)", "v1315 线下主题＋背景挂到舞台上"),
        ("function offAppearance(id)", "v1315 线下外观面板"),
        ("function offSkinSet(id,skin)", "v1315 两套主题切换"),
        ("t.skin=t.skin==='classic'?'classic':'glass';", "v1315 主题只认这两个"),
        ("function offBgPick(id)", "v1315 线下换背景（不压画质）"),
        ("function offFieldHTML(placeholder)", "v1315 线下输入框包了一层玻璃"),
        ("const PF_CID_PREFIX='pf:'", "v1315 真人好友转账走角色那一套"),
        ("function pfTransferView(m)", "v1315 pf 转账翻成角色转账的形状"),
        ("function pfTransferDetailAction(fid,mid,action)", "v1315 pf 收款/退还"),
        ("function pfHandleTransferRefund(m)", "v1315 对方退还之后钱退回来"),
        ("p.type==='style_update'||p.type==='transfer_refund'", "v1315 退还消息不当气泡显示"),
        ("c.p==='pfchat'?'pf_input':'ginput'", "v1315 真人好友 1v1 的输入框终于被绑上"),
        ("else if(c.p==='pfchat')sendPhoneFriend(c.id)", "v1315 回车也能发"),
        ("||c.p==='pfchat')afterGroupComposer(c);", "v1315 渲染完要去绑"),
        ("${pfPanelHTML(id)}`;}", "v1315 真人好友功能面板在输入框下面"),
        ("${pfGroupPanelHTML(gid)}`;}", "v1315 群聊功能面板也在下面"),
        ("onclick=\"genCharPrompt()\">✨ 一键生成", "v1315 新的朋友里的一键生成入口"),
    ...
    for marker, label in [
        ...
        (".offmsg.them .offbubble{padding:9px 15px 9px 22px;background:var(--offc-them);", "v1315 线下气泡走同一套轮廓"),
        (".offstage .offmsg .offbubble:before{", "v1315 线下那条细高光环"),
        (".offstage{--offc-me:#0a84ff;", "v1315 线下配色变量"),
        (".off-field:before,.offinput .send:before{", "v1315 输入框和按钮那圈挖空的环"),
        (".off-date-nav>.t{position:absolute!important;left:50%!important;", "v1315 线下昵称按整条栏居中"),
        (".off-reply-top{position:relative;height:27px;", "v1315 让TA回改成灰玻璃"),
        (".offstage.off-classic .offmsg .offbubble{clip-path:none!important;", "v1315 原来那套主题原样留着"),
        (".offstage.hasbg{background-size:cover;background-position:center;background-repeat:no-repeat;}", "v1315 壁纸真的铺满全屏"),
        (".offstage.hasbg:not(.off-classic) .cohab-settings{", "v1315 共同生活设置也变玻璃"),
        (".offstage.hasbg:not(.off-classic) .cohab-status-chip{position:relative;", "v1315 在家的状态也变玻璃"),
        (".offstage.hasbg:not(.off-classic) .cohab-debug-reply{", "v1315 共同生活的让TA回也变玻璃"),
...
def main() -> None:
    ...
    with TemporaryDirectory(prefix="private1301-verify-") as folder:
        with ZipFile(OUTPUT) as archive:

...
INSTALL_GUIDE_SOURCE = (
    SOURCE / "第二百九十五次安装_v1168_私人性能链继承修复_请先读.md"
)
DELIVERY = ROOT / "delivery-v1168-private295-performance-inheritance-candidate"
PACKAGE_NAME = "SmallPhone_v1168_PrivatePerformanceInheritance_iOS295_MacReady"
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
USER_ZIP = ROOT.parent / "小手机_v1168_私人版_iOS295_私人性能链继承修复_Mac待编译源码包.zip"
...
if "APP_VER='v1167 · 心动审判共同生活记忆修复版'" not in public_app:
    raise RuntimeError("public web baseline is no longer v1167")
...
with tempfile.TemporaryDirectory(prefix="smallphone-v1168-ios295-", dir=ROOT) as temp:
    ...
    require_tokens(
        shell,
        [
            "window.__NORTH_SHELL_BUILD__='1168'",
            "app.js?v=1168&r=v1168-private-performance-inheritance-1",
            "private-runtime-diagnostics.js?v=295",
        ],
        "private shell",
    )
    repair = (bundle / "repair.html").read_text(encoding="utf-8")
    require_tokens(repair, ["index.html?repair=1&v=1168"], "repair page")
    for destructive in [
    ...
    require_tokens(
        ...
        [
            ...
            "while(out.length<HEART_QUIZ_TOTAL&&calls<10)",
            "正在分批准备审判",
        ],
        "heart quiz",
    )
    ...
    require_tokens(
        overlay,
        [
            "295-private-performance-inheritance-v1",
    ...
    if project.count("CURRENT_PROJECT_VERSION = 295;") != 12:
        raise RuntimeError("private build 295 is not set on all target configurations")
    if project.count("MARKETING_VERSION = 1.0.295;") != 12:
        raise RuntimeError("private version 1.0.295 is not set consistently")
    ...
    for guide in [main_guide, install_guide]:
        require_tokens(
            guide,
            ["v1168", "1.0.295 (295)", "原生桥 35", "不要先删除"],
            "Mac guide",
        )

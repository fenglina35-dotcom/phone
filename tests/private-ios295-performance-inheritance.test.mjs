...
test('private identifiers advance to v1254 and iOS 362 while public stays v1254', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1254'/);
  assert.match(index, /app\.js\?v=1254&r=v1254-private-sync-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=336/);
  assert.match(app, /APP_VER='v1254 · 私人同步网页十项修复'/);
  assert.match(overlay, /336-daily-file-backup/);
  assert.match(webview, /1\.0\.362 \(362\)/);
  assert.match(bridge, /private static let build = "1\.0\.362 \(362\)"/);
  assert.match(bridge, /static let contractVersion = 38/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 362;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.362;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1253 · 真人好友撤回入口与语音语言锁'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1253'/);
...
test('Mac guide identifies the current private v1254 iOS362 source', () => {

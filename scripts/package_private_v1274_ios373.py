...
def main() -> None:
    ...
    validate(files)
    house_root = ROOT.parent / "温馨小家3D/web/dist"
    for asset in house_root.rglob("*"):
        if not asset.is_file(): continue
        rel = asset.relative_to(house_root).as_posix()
        actual = files[BUNDLE + "games/cozy-home/" + rel]
        expected = asset.read_bytes()
        if rel == "index.html" and b'src="private-bridge.mjs"' not in expected:
            actual = actual.replace(b'<script type="module" src="private-bridge.mjs"></script>', b'')
        if asset.suffix.lower() in {".html",".mjs",".js",".json",".txt",".md",".css",".svg",".webmanifest"}:
            actual=actual.replace(b"\r\n",b"\n");expected=expected.replace(b"\r\n",b"\n")
        assert actual == expected, "house resource differs: " + rel

...
(function(){
  ...
  function launchPay(id){var o=(S.food.orders||[]).find(function(x){return x.id===id;});if(!o||!o.payUrl)return;try{window.open(o.payUrl,'_blank','noopener');toast('正在交给系统打开官方付款页面');}catch(e){toast('没有打开付款页面，请使用订单里的官方二维码');}}
  ...
  async function chooseOptions(c,query,offer,intent,resumeDetail){
    ...
    var resumeText=resumeDetail&&resumeDetail.kind==='minimum_order'?'\n上一轮平台真实结算限制：'+text(resumeDetail.reason,180)+'。用户已经回答是否继续；若同意增加同款，quantity 至少为'+Math.max(1,+resumeDetail.minimumQuantity||1)+'，不要再次追问。':'';
  ...
  function requestRoleClarification(c,detail){
    ...
    var kind=detail.kind==='minimum_order'?'minimum_order':detail.kind==='history_repeat'?'history_repeat':'options';
    r.roleClarifications[c.id]={kind:kind,query:text(detail.query,120),offer:safeOffer(offer),userMessages:(detail.userMessages||[]).map(function(x){return text(x,240);}).filter(Boolean).slice(-10),reason:text(detail.reason,180),minimumQuantity:Math.max(0,+detail.minimumQuantity||0),at:Date.now()};
    roleSystemNotice(kind==='minimum_order'?'真实外卖已暂停：该门店未达到起送金额，等待你决定是否增加同款数量或换店。':kind==='history_repeat'?'真实外卖已暂停：历史订单含本次没有明确要求的额外商品，等待你确认是否整单复购。':'真实外卖已暂停：需要你确认平台可选规格后再继续，尚未加购或下单。','info');save();
    if(!c.blocked){var fact=kind==='minimum_order'?'[系统：这次真实外卖没有进入结算或付款，平台确认当前商品金额未达到门店起送要求，加购已停止或撤回。当前商品：'+offer.merchant+' / '+offer.name+'。真实限制：'+text(detail.reason,160)+'。请严格按你自己的完整人设和说话习惯，自然告诉'+S.me.name+'这个门店的起送限制，并只问一个必要问题：是否把同款增加到至少'+Math.max(2,+detail.minimumQuantity||2)+'份，还是换店。不要复述技术错误，不要声称已下单或付款。对方同意增加后系统会继续同一商品；对方要求换商品时系统会重新搜索。]':kind==='history_repeat'?'[系统：平台找到一笔真实历史整单，里面包含本次明确要求的全部商品，但还包含本次没有明确要求的额外商品。当前商家/主商品：'+offer.merchant+' / '+offer.name+'。历史整单事实：'+text(detail.reason,180)+'。系统尚未重新加购、下单或付款。请严格按你自己的完整人设和说话习惯，自然告诉'+S.me.name+'多出的商品，并只问一个必要问题：是否愿意原样复购整个历史套餐。句式由你自己决定，不要复述技术错误。只有对方明确同意才会继续整单复购；若拒绝，系统会按本次需求重新搜索。]':'[系统：这次真实外卖已安全暂停，尚未加购、下单或付款。当前商品：'+offer.merchant+' / '+offer.name+'。平台当前真实可选规格：'+clarificationOptions(offer)+'。未能继续的真实原因：'+text(detail.reason,160)+'。请严格按你自己的完整人设和说话习惯，只向'+S.me.name+'自然说明必要事实并问一个最必要的澄清问题；句式由你自己决定，不要照抄或复述技术错误，不要声称已经选好或下单。如果你想选热饮而当前商品没有热饮，只需自然问是否换一种，不要强迫对方接受冷饮。对方回答后系统会继续同一轮；若对方说出另一种商品，系统会放弃当前商品并重新搜索一次。]';scheduleReply(c.id,fact);}return true;
  }
  ...
  async function roleRequest(cid,query){
    ...
    var proactiveOrder=/^主动关心\s*[:：|]/.test(query||''),clarification=r.roleClarifications[cid]||null,historyApproval=clarification&&clarification.kind==='history_repeat'&&/^继续确认历史订单$/.test(text(query,40)),changeProduct=clarification&&/^(?:换品|重新搜索)\s*[:：|]/.test(query||'');if(proactiveOrder)query=text(String(query).replace(/^主动关心\s*[:：|]\s*/,''),120);if(changeProduct){query=text(String(query).replace(/^(?:换品|重新搜索)\s*[:：|]\s*/,''),120);delete r.roleClarifications[cid];clarification=null;}else if(clarification)query=clarification.query||query;
    if(!query){resultReply(c,null,'没有提供可搜索的餐品或店铺');return true;}
    roleRequests[cid]={status:'reconciling'};var recent=await reconcileRecentRoleOrder(cid);if(recent.order){delete roleRequests[cid];var notice=recent.error?'上一笔已付款的真实外卖状态暂时无法向平台核对，为避免重复没有再次下单。请检查电脑外卖服务后重试。':'已有一笔已付款的真实外卖仍在接单、制作或配送；为避免重复没有再次下单。';roleSystemNotice(notice,'info');return true;}
    var previousAttempt=r.roleAttempts[cid],intent=roleRequestIntent(c,query,previousAttempt);if(proactiveOrder)intent.autonomous=true;if(clarification){intent.userMessages=(clarification.userMessages||[]).concat(intent.userMessages||[]).slice(-12);intent.autonomous=proactiveOrder||/(?:随便(?:点|挑|选|买)|你(?:自己)?(?:决定|挑|选|看着(?:点|买|办))|按你(?:自己)?想的|你来定|都可以|都行|都喜欢|听你的)/.test((intent.userMessages||[]).join('\n'));intent.confirmedHistoricalSuperset=historyApproval===true;}var attempt={query:query,userMessages:intent.userMessages,autonomous:intent.autonomous,proactive:proactiveOrder,startedAt:Date.now(),endedAt:0,status:'running',error:''};roleRequests[cid]=attempt;r.roleAttempts[cid]=attempt;save();
    var order=null;try{var picked;if(clarification&&clarification.offer&&clarification.offer.quoteExpiresAt>Date.now()+5000){var resumedOffer=safeOffer(clarification.offer),resumedChoice=await chooseOptions(c,query,resumedOffer,intent,clarification);if(clarification.kind==='minimum_order'&&clarification.minimumQuantity>1)resumedChoice.quantity=Math.max(resumedChoice.quantity,clarification.minimumQuantity);picked={offer:resumedOffer,choice:resumedChoice};}else{var offers=await realSearch(query,{roleId:cid});picked=await chooseOfferWithOptions(c,query,offers,intent);}var offer=picked.offer,choice=picked.choice;delete r.roleClarifications[cid];order=await createOrder(offer,{roleId:cid,quantity:choice.quantity,selectedOptions:choice.selectedOptions,confirmedHistoricalSuperset:intent.confirmedHistoricalSuperset===true});await payOrder(order);attempt.status='completed';attempt.endedAt=Date.now();save();resultReply(c,order,'');return true;}
    catch(e){attempt.status='failed';attempt.endedAt=Date.now();attempt.error=text(e.message||'真实外卖操作失败',180);attempt.failureKind=/打烊|休息中|门店休息|暂停营业|不在营业时间/.test(attempt.error)?'shop_closed':/最低起送金额|未达到起送金额/.test(attempt.error)?'minimum_order':'';if(e&&e.deliveryClarification){attempt.status='awaiting_clarification';save();requestRoleClarification(c,e.deliveryClarification);return true;}if(attempt.failureKind==='minimum_order'&&offer){var minimumMatch=attempt.error.match(/至少需要(\d+)份/);attempt.status='awaiting_clarification';save();requestRoleClarification(c,{kind:'minimum_order',query:query,offer:offer,userMessages:intent.userMessages,reason:attempt.error,minimumQuantity:+(minimumMatch&&minimumMatch[1]||0)});return true;}if(clarification&&!/验证|登录|地址|网络|超时|服务|报价|购物车|打烊|休息|暂停营业/.test(attempt.error)){clarification.reason=attempt.error;clarification.at=Date.now();r.roleClarifications[cid]=clarification;attempt.status='awaiting_clarification';save();requestRoleClarification(c,clarification);return true;}delete r.roleClarifications[cid];save();resultReply(c,order,attempt.error,attempt.failureKind);return true;}
    finally{delete roleRequests[cid];}
  }
  function rolePreludeAllowed(line){line=text(line,160);if(!line||/已经|订单|收银台|付款|支付|骑手|接单|配送|送到|优惠券|实付|待付|下单|点好|买好|订好|付好/.test(line))return false;return /等|稍等|等我|我来|我去|我先|给我一下|看看|找找|挑挑|选选|别急|^(?:好|行|可以|知道了|明白了|那我|这就|继续|马上)/.test(line);}
  ...
  function rolePrompt(c){
    ...
    return '\n\n# 真实外卖能力与硬边界\n真实外卖已开启，但你始终保留自己的判断和意愿：可以按你的人设、关系、情绪与当下处境同意，也有权直接拒绝给'+S.me.name+'点单。拒绝时只需用你自己的口吻正常说明，不要输出任何外卖标签，也不会启动真实订单。你可以按本人判断主动使用，也可以在'+S.me.name+'明确让你点某样东西时搜索。若最近真实聊天或记忆明确说明'+S.me.name+'没吃饭、不好好吃饭或拿饮料零食代替正餐，你可以不等ta索要，按本人意愿主动选择并创建一笔待付款外卖；决定这样做时使用 [真实外卖|主动关心:具体品牌和具体商品]，这表示选择权来自你本人，不能再追问品牌口味，但过敏和忌口必须遵守。没有这类真实依据时不得凭空主动下单。若对方只是泛泛说“想喝奶茶”“想吃点东西”，还没说明具体品牌/门店或具体商品，也没有明确让你自行决定，就必须先按你自己的语气自然问清楚并等待回答；这一轮绝不能输出真实外卖标签。若对方明确说“随便点、你选、都可以、都喜欢、按你想的”等授权你自行决定，这就已经是完整答案：不要再追问品牌、商品、口味、糖度、温度或规格，直接结合奶茶、主食、咖啡和 KFC 的全部已保存偏好、你的人设、关系和当下处境，自主选择类别、品牌、具体商品及可用规格；偏好只供参考，过敏和忌口仍必须遵守。只有你自己同意，并且对方已经说清品牌/门店与商品、明确让你自行决定，或满足上面的主动关心依据，才可以开始真实搜索。\n真正决定尝试时，必须先发一句符合你本人语气的简短可见消息，让'+S.me.name+'知道你正在找，例如自然表达“等我一下”“我去看看”；紧接着再单独输出一行 [真实外卖|具体搜索词]，主动关心则使用上面的专用格式。搜索词应写你真正决定搜索的品牌/门店和具体商品；只有用户亲口明确提出的糖度、冰度、温度、口味、辣度、份量或加料才是硬条件，角色自己为了搜索补写的词不能反过来当成用户硬条件。系统会优先直达成功点过的同款；没有路线或门店打烊时，会自动进行一次受限搜索并最多检查三家匹配门店，在店内再次搜索准确商品。用户明确要求的每一件商品都是不可遗漏的清单项：必须把品牌和全部单品写进同一条具体搜索词，用“加”分隔；系统会逐项加入并在确认订单页再次核对，达到起送价也绝不能提前停止或少买。KFC 只允许逐件选择单品，严禁套餐、组合、全家桶和用增加数量代替其他商品。严格匹配用户真正明确的商品和真实规格后才进入支付宝待付款页，由用户本人完成付款。\n开始操作的可见消息只能表达“准备找/准备看看”，绝不能提前声称已经找到、用了优惠券、下单、付款、骑手接单或送达；只有收到后续真实操作结果后才能说对应事实。找不到用户真正明确的品牌、商品或规格时必须如实失败，不能换品牌、换商品或改口味，也不能改走虚拟外卖。不要再使用 [点外卖|餐品|价格]。';
  }
  function preferenceHierarchyPrompt(){return '\n\n# 外卖偏好分层规则（必须执行）\n后台偏好中的每一个「」条目都是独立候选，不是一整句搜索词。必须先判断本次类别，再选一家门店，再逐件选择商品：奶茶只用奶茶门店和奶茶商品；咖啡只用咖啡门店和咖啡商品；KFC 门店固定为肯德基，并逐件选择单品，严禁套餐；其余一律归为主食。主食的“品牌、菜系或品类候选”只用来选择一个下单方向：例如「曼玲粥」是品牌，「中餐」「面食」是菜系或品类，它们都不是要加入购物车的商品；确定方向后仍需选择一家真实营业门店。主食允许套餐或单点：若'+S.me.name+'明确说套餐或单点，必须照做；未说明时你可以按人设和当下需求自主决定，也可以在确有必要时自然询问。\n“门店候选”只能用于确定一家商家，绝不能当成要买的食物；“商品候选”必须一件一件加入，绝不能把多个候选拼成一次搜索。你主动决定时，也只能从某一类别中选一家门店和少量具体商品，不能把后台整排偏好全部下单。'+S.me.name+'本轮明确说出的品牌、商品、规格、套餐或单点要求永远高于所有后台偏好。';}
  function rolePromptWithPreferenceHierarchy(c){var base=rolePrompt(c);return base.indexOf('# 真实外卖能力与硬边界')>=0?base+preferenceHierarchyPrompt():base;}

  foodState();deliveryPreferences();setTimeout(function(){(S.food.orders||[]).forEach(armEtaReminder);},1200);
  ...
  /* 订单状态只在用户点击“刷新平台状态”时读取。页面刷新、回到前台或
     网络恢复都不得静默启动/跳转外卖浏览器，否则会打断购物车并增加
     平台验证触发。 */
})();

...
test('both private entry files and every iOS target carry build 333', () => {
  ...
  assert.match(bridge, /1\.0\.342 \(342\)/);
  assert.equal(
    (project.match(/CURRENT_PROJECT_VERSION = 342;/g) || []).length,
    12
  );
  assert.equal(
    (project.match(/MARKETING_VERSION = 1\.0\.342;/g) || []).length,
    12
  );
});

...
test('v1178 shared theater asset is byte-identical and loaded after app core',()=>{
  ...
  assert.match(webHtml,/app\.js\?v=1253[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1253&r=v1184-ios-web-crash-cohab-turn-keyboard-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1254[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1254[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1254&r=v1184-ios-web-crash-cohab-turn-keyboard-1"/);
...
test('private artifact identity advances independently to v1254 and iOS 1.0.362 (362)',()=>{
  assert.match(privateApp,/const APP_VER='v1254 · 私人同步网页十项修复'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=336/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.362 \(362\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 362;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.362;/g)||[]).length>=12);
});

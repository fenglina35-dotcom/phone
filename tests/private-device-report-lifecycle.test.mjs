...
test('private build and bundled recovery page advance together', () => {
  ...
  assert.match(webView, /smallPhone\.webContentTerminationTimes\.v25\.build322/);
  assert.match(privateApp, /APP_VER='v1200 · 回复接力互斥版'/);
  assert.equal(privateAlias, privateIndex);
  assert.match(privateIndex, /window\.__NORTH_SHELL_BUILD__='1200'/);
  assert.match(privateIndex, /app\.js\?v=1200/);
  assert.match(privateIndex, /private-runtime-diagnostics\.js\?v=322/);
  assert.match(privateRepair, /index\.html\?repair=1&v=1200/);
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!==\'1200\'/);
});

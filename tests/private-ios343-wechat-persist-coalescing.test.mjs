...
test('private identity remains v1287 and iOS 380 while public advances independently', () => {
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!=='1287'/);
  assert.match(privateApp, /APP_VER='v1287 · 抖音全屏作品与配乐（私人）'/);
  for (const html of [privateIndex, privateAlias]) {
    assert.match(html, /window\.__NORTH_SHELL_BUILD__='1287'/);
    assert.match(html, /app\.js\?v=1287&r=v1287-private-dy-fullscreen-1/);
    assert.match(html, /private-runtime-diagnostics\.js\?v=339/);
  }
  assert.match(privateRepair, /index\.html\?repair=1&v=1287/);
  assert.match(swift, /1\.0\.380 \(380\)/);
  ...
  assert.match(publicIndex, /window\.__NORTH_SHELL_BUILD__='1286'/);

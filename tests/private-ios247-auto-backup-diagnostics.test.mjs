...
test('private iOS 252 overlay owns the disable marker and cancels automatic scheduling', async () => {
...
test('manual backup and both restore actions remain free of the automatic-disable guard', () => {
  for (const name of [
    ...
    assert.doesNotMatch(source, /252-wardrobe-time-delivery-v1/);
  }
  assert.doesNotMatch(
    overlay,
...
test('native recovery UI stays outside WebKit and carries the private 252 identity', () => {
  ...
  assert.match(bridge, /private static let build = "1\.0\.252 \(252\)"/);

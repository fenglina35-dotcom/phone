...
test('an explicit request to post the previous announcement photo cannot silently become text-only', () => {
  ...
  assert.match(app, /function consumeMomentCommands\(content,c,opt,outcome\)[\s\S]*?postRoleMoment\(c,body,opt\)[\s\S]*?return opt&&opt\.preserveText\?out:stripPostedMomentEcho\(out,posted\);\s*\}/);
  assert.match(app, /consumeMomentCommands\(content,c,\{toast:true,userText:_userText,preserveText:_rawOutput\},_replyActionOutcome\)/);
});

...
test('the web shell and service worker require the hotfix while private files stay outside its entry path',()=>{
  assert.match(shell,/web-hotfix\.js\?v=1122&r=sticker-avatar-login-3/);
  assert.match(hotfix,/sw\.js\?v=1122&r=v1122-sticker-avatar-login-hotfix-3/);
  assert.match(worker,/kind:'hotfix'/);
  assert.match(worker,/north-shell-v1122-sticker-avatar-login-3/);

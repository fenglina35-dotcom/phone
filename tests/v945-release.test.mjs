...
test('v1184 web files use one cache-busting build number',()=>{
  ...
  assert.match(app,/const url='sw\.js\?v=1330&r=v1330-web-delivery-context-1'/);
  assert.match(html,/__NORTH_SHELL_BUILD__='1330'/);
  assert.match(html,/app\.js\?v=1330/);
  assert.match(sw,/const BUILD='1330'/);
  assert.match(sw,/north-shell-v1330-web-delivery-context-1/);
...
test('the private iOS source embeds private web v1189 and keeps native build 315 before repackaging',()=>{
  ...
  assert.equal((xcode.match(/MARKETING_VERSION = 1.0.399;/g)||[]).length,12);
  assert.equal((xcode.match(/CURRENT_PROJECT_VERSION = 399;/g)||[]).length,12);

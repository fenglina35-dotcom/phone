"""Create the private v1277 / iOS 376 Mac-source overlay package.
...
PREFIX = "SmallPhone_v1277_iOS376_Private/"
OUTPUT = ROOT.parent / "SmallPhone_v1277_iOS376_MacSource.zip"
...
def validate(files: dict[str, bytes]) -> None:
    ...
    assert "uploader.upload(" in bridge and "fromFile: file.url" in bridge
...
def main() -> None:
    ...
    files["SOURCE_STATE.json"] = json.dumps({"sourceCommit":text(git("rev-parse","HEAD")).strip(),"upstreamBaseline":"a3a95087cd13989a9e02d8d69b5a4273bb201ee3","houseSource":"df56e66a4f24efed42e4c37ef7d1908deb95df03","privateWeb":"v1277","privateIOS":"1.0.376 (376)","bridge":39,"macBuildVerified":False,"realIPhoneVerified":False,"kind":"Mac-source-not-IPA","preserved":["all-public-v1275-features","all-private-v1277-features","private-backup-single-upload-progress","web-cloud-existing-row-update","web-cloud-persistent-progress","private-backup-commit-timeout-720s","online-memory-recall-controls","offline-memory-scroll-restore","cozy-private-native-bridge","diagnostic-copy-freshness","native-probe-lifecycle","house052-complete-assets","first-person-head-tracking","isolated-mobile-shader-warmup","warm-gpu-resource-release","female-standing-pose"]},ensure_ascii=False,indent=2).encode("utf-8")
    ...
    with TemporaryDirectory(prefix="private1277-verify-") as folder:
        with ZipFile(OUTPUT) as archive:

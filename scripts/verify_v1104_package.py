...
package = root / "delivery-v1104-private226-final" / "SmallPhone_v1104_RolePushOutputSafety_iOS226_MacReady.zip"
...
with ZipFile(package) as archive:
    ...
    guide = next((name for name in names if name.endswith("第二百二十六次安装_v1104_后台主动消息安全_请先读.md")), "")
    if not guide:
        raise RuntimeError("missing v1104 install readme")
    prefix = guide.split("第二百二十六次安装", 1)[0] + "PhoneCompanionTest/PhoneWeb.bundle/"
    ...
    if "window.__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.226 (226)'" not in webview:
        raise RuntimeError("bundled private build marker mismatch")
    if project.count("CURRENT_PROJECT_VERSION = 226;") != 12 or project.count("MARKETING_VERSION = 1.0.226;") != 12:
...
print("v1104 package structure, byte identity, version and safety checks passed")

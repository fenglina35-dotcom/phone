...
test('private identifiers advance to v1234 and iOS 355 while public stays v1234', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1234'/);
  assert.match(index, /app\.js\?v=1234&r=v1234-huawei-edge-backup-download-1/);
  ...
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1234'/);
...
test('Mac guide distinguishes v1234 source sync from the unchanged iOS355 package', () => {
  ...
  assert.match(mac, /^# v1234 .*仅同步内置网页源码/);

...
test('thermal pressure and repeated termination stop WebKit self-reload loops', () => {
  ...
  assert.match(webView, /webContentTerminationTimes\.v25\.build333/);
...
test('private entries carry diagnostics 334 and every iOS target carries build 347', () => {
  ...
  assert.match(bridge, /1\.0\.347 \(347\)/);
  assert.equal(
    (project.match(/CURRENT_PROJECT_VERSION = 347;/g) || []).length,
    12
  );
  assert.equal(
    (project.match(/MARKETING_VERSION = 1\.0\.347;/g) || []).length,
    12
  );
});

...
test('private v1315 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1314[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1314&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1315[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1315[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1315&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1315 and iOS 1.0.391 (391)',()=>{
  assert.match(privateApp,/const APP_VER='v1315 · 线下玻璃两套主题、真人好友转账与发送键、微信换背景（私人）'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.391 \(391\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 391;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.391;/g)||[]).length>=12);
});

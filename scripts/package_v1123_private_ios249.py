...
if tracked_status and not ALLOW_DIRTY_PACKAGE:
    raise RuntimeError("tracked working tree is dirty; commit private iOS 249 first")
...
with tempfile.TemporaryDirectory(prefix="smallphone-v1123-ios249-", dir=ROOT) as temp:
    ...
    for token in [
        ...
        'private-runtime-diagnostics.js?v=249',
        "app.js?v=1123",
    ]:
    ...
    if project.count("CURRENT_PROJECT_VERSION = 249;") != 12:
        raise RuntimeError("private build 249 is not set on all targets/configurations")
    if project.count("MARKETING_VERSION = 1.0.249;") != 12:
        raise RuntimeError("private version 1.0.249 is not set consistently")
    if "CURRENT_PROJECT_VERSION = 248;" in project:
        raise RuntimeError("private project still contains build 248")
    if "MARKETING_VERSION = 1.0.248;" in project:
        raise RuntimeError("private project still contains version 1.0.248")
    ...
    if "私人 iOS 1.0.249 (249)" not in main_guide:
        raise RuntimeError("Mac guide does not identify private iOS 249")

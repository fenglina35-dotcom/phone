"""Create the private v1299 / iOS 383 Mac-source overlay package.
...
PREFIX = "SmallPhone_v1299_iOS383_Private/"
OUTPUT = ROOT.parent / "SmallPhone_v1299_iOS383_MacSource.zip"
...
def validate(files: dict[str, bytes]) -> None:
    ...
    for marker, label in [
        ...
        ("backdrop-filter:brightness(1.62) saturate(1.75)", "v1294 高光跟着背景变颜色"),
...
def main() -> None:
    ...
    files["SOURCE_STATE.json"] = json.dumps({"sourceCommit":text(git("rev-parse","HEAD")).strip(),"upstreamBaseline":"bd6a30014c2f87cb4f8553a8015f5fe90cd5068d","houseSource":"df56e66a4f24efed42e4c37ef7d1908deb95df03","privateWeb":"v1299","privateIOS":"1.0.383 (383)","bridge":41,"macBuildVerified":False,"realIPhoneVerified":False,"kind":"Mac-source-not-IPA","preserved":["all-public-v1298-features","role-moment-idb-cover-rehydration","internal-app-decision-hidden-at-all-delivery-boundaries","all-private-v1281-features","private-backup-4mib-object-chunks","private-backup-small-manifest-commit","private-backup-checksum-restore","private-backup-legacy-fallback","private-backup-single-upload-progress","web-cloud-existing-row-update","web-cloud-persistent-progress","private-backup-commit-timeout-1920s","private-call-smart-home-tag-sanitizer","private-call-inline-translation-deduplication","online-memory-recall-controls","offline-memory-scroll-restore","cozy-private-native-bridge","diagnostic-copy-freshness","native-probe-lifecycle","house052-complete-assets","first-person-head-tracking","isolated-mobile-shader-warmup","warm-gpu-resource-release","female-standing-pose","imessage-sms-page","per-contact-sms-background","traced-hairline-glass","thin-bubble-tail","bare-photo-messages","sms-typing-dots","sms-multi-bubble","global-context-everywhere","role-sets-chat-background","sms-photo-vision"]},ensure_ascii=False,indent=2).encode("utf-8")
    ...
    with TemporaryDirectory(prefix="private1299-verify-") as folder:
        with ZipFile(OUTPUT) as archive:

...
assert.match(source, /APP_VER='v1058 · 共同生活通话、存储散热、转账回复与约会输入修复版'/);
...
assert.match(functionSource("cinemaDirectPlayOnly"), /cinemaAndroidBrowser\(\)[\s\S]*256\*1024\*1024/);
assert.match(functionSource("cinemaSaveLocalVideo"), /direct\?'missing':'saving'/);
assert.match(functionSource("cinemaSaveLocalVideo"), /长视频已直接播放，不再复制到浏览器存储/);
assert.match(functionSource("cinemaVideoRetryCompatible"), /avc1\|avc3[\s\S]*mp4a\|aac/);
assert.match(functionSource("cinemaRetryOriginalVideo"), /videoRetryCount>=1[\s\S]*URL\.createObjectURL\(_cin\.videoFile\)/);
assert.match(functionSource("cinemaVideoPlaybackError"), /cinemaRetryOriginalVideo\(v,info\)/);
assert.match(functionSource("cinemaVideoCompatibilityHint"), /H\.264 High Profile[\s\S]*yuv420p[\s\S]*faststart/);
assert.match(functionSource("cinemaVideoPlaybackError"), /两小时的时长本身不会导致格式错误/);
...
assert.match(html, /app\.js\?v=1058/);

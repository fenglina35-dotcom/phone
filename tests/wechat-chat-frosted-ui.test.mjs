...
assert.match(app,/chatVoiceButtonIcon\(\)[^\n]*r="13\.4"/,'voice control uses the smile control diameter');
assert.match(glass,/\.chat-function-toggle>span\{width:28px;height:28px;[^}]*border:1\.8px solid currentColor/,'plus control uses the smile control diameter and visual weight');
assert.match(glass,/\.chat-emoji-toggle svg\{width:37px!important;height:37px!important/,'the approved smile size stays unchanged');
...
assert.match(app,/function wxPlusIcon\(\)[^\n]*<line x1="17" y1="9\.5" x2="17" y2="24\.5"\/><line x1="9\.5" y1="17" x2="24\.5" y2="17"\/>/,'the WeChat header plus uses two lines crossing at the exact circle center');

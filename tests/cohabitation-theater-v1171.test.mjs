...
test('private v1327 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1326[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1326&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1327[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1327[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1327&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1327 and iOS 1.0.397 (397)',()=>{
  assert.match(privateApp,/const APP_VER='v1327 · 外卖嵌套商品名解析修复'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.397 \(397\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 397;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.397;/g)||[]).length>=12);
});

...
test('private identifiers remain v1301 and iOS 384 while public advances independently', () => {
  ...
  assert.match(index, /app\.js\?v=1301&r=v1301-private-imsg-5/);
  ...
  assert.match(webview, /1\.0\.384 \(384\)/);
  assert.match(bridge, /private static let build = "1\.0\.384 \(384\)"/);
  assert.match(bridge, /static let contractVersion = 41/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 384;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.384;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1300 · 磨砂＋细高光、共同生活状态、背景画质'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1300'/);
...
test('Mac guide identifies the current private v1301 iOS384 source', () => {

...
function fixture(store=new Map()){
 ...
 const ctx={console,Date:class extends Date{static now(){return now;}},setTimeout(fn){timers.push(fn);return timers.length;},clearTimeout(){},setInterval(fn){intervals.push(fn);},document:{hidden:false,images:[],scripts:[],addEventListener(n,fn){handlers['doc:'+n]=fn;}},navigator:{userAgent:'fixture'},localStorage:{getItem:k=>store.get(k)||null,setItem:(k,v)=>store.set(k,v)},addEventListener(n,fn){handlers[n]=fn;},__NORTH_SHELL_BUILD__:'1260'};ctx.window=ctx;vm.createContext(ctx);vm.runInContext(code,ctx);return {ctx,store,handlers,timers,intervals,advance(ms){now+=ms;},report(){return JSON.parse(ctx.NorthBrowserDiagnostics.report());}};
}
test('diagnostics persist bounded sanitized stack frames and survive reload',()=>{
 const f=fixture();const error={name:'RangeError',message:'Maximum call stack size exceeded PRIVATE_TEXT token=secret',stack:'RangeError PRIVATE_TEXT\n'+Array(30).fill('    at cohabAdvance (https://private.invalid/app.js?v=1260&key=secret:6967:42)').join('\n')};

...
test('private iOS 290 overlay owns the disable marker and cancels automatic scheduling', async () => {
  const { context, calls } = makeAutoBackupHarness();
  assert.equal(
    context.__SMALL_PHONE_PRIVATE_RUNTIME__,
    '290-proactive-heart-v1'
  );
...
test('manual backup and both restore actions remain free of the automatic-disable guard', () => {
  for (const name of [
    ...
    assert.doesNotMatch(source, /290-proactive-heart-v1/);
  }
  assert.doesNotMatch(
    overlay,
...
test('native recovery UI stays outside WebKit and carries the private 290 identity', () => {
  ...
  assert.match(bridge, /private static let build = "1\.0\.290 \(290\)"/);

...
DELIVERY = ROOT / "delivery-v1045"
PACKAGE_NAME = "SmallPhone_v1045_WeChatStepsDeliveryRoutes_iOS165_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百六十五次安装_v1045_微信运动与外卖精准直达整合_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1045":
...
for token in ["APP_VER='v1045 · 微信运动与外卖精准直达整合版';", "renderWxSteps", "callApplyBackdrop", "callReplayCanvasImage", "off-date-nav"]:
    if token not in app_text:
        raise RuntimeError(f"v1045 bundled app repair missing: {token}")
delivery_text = (bundle / "delivery.js").read_text(encoding="utf-8")
for token in ["PREF_CONFIG", "saved_routes", "shop_closed", "全部打烊或休息中"]:
    if token not in delivery_text:
        raise RuntimeError(f"v1045 bundled delivery repair missing: {token}")
if "FAMILY PRIVILEGE" not in (bundle / "wechat-me.js").read_text(encoding="utf-8"):
    raise RuntimeError("v1045 family card repair missing")
if "<string>1045</string>" not in (bundle / "Info.plist").read_text(encoding="utf-8"):
    raise RuntimeError("PhoneWeb.bundle Info.plist is not v1045")

project_text = (STAGING / "PhoneCompanionTest.xcodeproj/project.pbxproj").read_text(encoding="utf-8")
if project_text.count("CURRENT_PROJECT_VERSION = 165;") != 12 or project_text.count("MARKETING_VERSION = 1.0.165;") != 12:
    raise RuntimeError("private iOS version is not consistently 1.0.165 (165)")
webview_text = (STAGING / "PhoneCompanionTest/LocalPhoneWebView.swift").read_text(encoding="utf-8")
if "__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.165 (165)'" not in webview_text:
    raise RuntimeError("private build marker is not 1.0.165 (165)")
...
if list(DELIVERY.glob("*.zip")) != [ZIP_PATH]:
    raise RuntimeError("delivery-v1045 must contain exactly one ZIP")

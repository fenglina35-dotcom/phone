...
test('public web v1184 and private web v1188 remain compatible with native iOS 1.0.314', () => {
  ...
  assert.match(bundleInfo, /<string>1188<\/string>/);
  assert.match(localWebView, /1\.0\.314 \(314\)/);
  assert.equal((project.match(/CURRENT_PROJECT_VERSION = 314;/g) || []).length, 12);
  assert.equal((project.match(/MARKETING_VERSION = 1\.0\.314;/g) || []).length, 12);
});

...
test('private v1274 inherits the current web theater without dropping its private shell',()=>{
  ...
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1274[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1274[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1274&r=v1272-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1274 and iOS 1.0.373 (373)',()=>{
  assert.match(privateApp,/const APP_VER='v1274 · 私人版温馨小家材质稳定修复'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.373 \(373\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 373;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.373;/g)||[]).length>=12);
});

...
(function(){
  ...
  async function chooseOptions(c,query,offer,intent){
    ...
    var raw=await chatAPI([{role:'system',content:'你是「'+c.name+'」本人，只做淘宝闪购真实规格选择，不要对用户说话。注意：角色生成的检索词不是用户原话。用户本次明确说出的杯型、份量、糖度、冰度、温度、口味、辣度、搭配和加料都是硬条件；真实选项缺少任意一项时必须返回 matched:false，绝不能擅自改规格或用相近选项替代。但若本轮已获得自主选择授权，除过敏与忌口外，上述规格都由你按人设和可用选项自主决定，不得把角色检索词或长期偏好当成硬条件。用户没说清的部分才参考全部长期外卖偏好和你的人设。不喜欢、忌口和过敏项必须避开。每个 required 组必须选择，只能使用真实 id。matched:false 的 reason 必须具体写出缺少哪一项，并列出平台现有的最接近选项，不能只写“规格不能完整满足”。只返回严格 JSON：{"matched":true,"quantity":1,"selectedOptions":{"组选项id":"选择id"},"reason":""}；多选组用数组；不能满足时返回 {"matched":false,"quantity":1,"selectedOptions":{},"reason":"缺少的规格及现有选项"}。'},{role:'user',content:roleIntentPrompt(intent)+'\n全部长期外卖偏好（非硬条件；过敏和忌口除外）：\n'+preferenceText()+'\n已严格匹配的真实商品：'+offer.merchant+' / '+offer.name+'\n平台真实规格：'+JSON.stringify(groups)}],{max:420,temp:.1});
  ...
  async function roleRequest(cid,query){
    ...
    var previousAttempt=r.roleAttempts[cid],intent=roleRequestIntent(c,query,previousAttempt),attempt={query:query,userMessages:intent.userMessages,autonomous:intent.autonomous,startedAt:Date.now(),endedAt:0,status:'running',error:''};roleRequests[cid]=attempt;r.roleAttempts[cid]=attempt;save();
    var order=null;try{var offers=await realSearch(query,{roleId:cid}),offer=await chooseOffer(c,query,offers,intent);await loadOfferOptions(offer);var choice=await chooseOptions(c,query,offer,intent);order=await createOrder(offer,{roleId:cid,quantity:choice.quantity,selectedOptions:choice.selectedOptions});pushRoleOrderCard(c,order);await payOrder(order);attempt.status='completed';attempt.endedAt=Date.now();save();resultReply(c,order,'');return true;}

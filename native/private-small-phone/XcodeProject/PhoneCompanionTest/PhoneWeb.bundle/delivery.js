...
(function(){
  ...
  function openSettings(){
    ...
    openModal('<div class="delivery-settings"><div class="delivery-settings-head"><div><small>DELIVERY CONTROL</small><h3>淘宝闪购真实外卖</h3></div><button onclick="closeModal()">×</button></div><div class="delivery-notice">只显示淘宝闪购真实返回的商家、商品、价格和平台状态；失败时不会用虚拟订单代替。</div>'+noticeHtml+'<label class="delivery-toggle"><span><b>开启真实外卖</b><small>默认关闭；开启后外卖软件和角色只创建淘宝闪购真实订单</small></span><input type="checkbox" '+(r.enabled?'checked':'')+' onchange="deliverySetEnabled(this.checked)"><i></i></label><div class="delivery-notice"><b>付款方式：支付宝本人确认</b><br>角色可以按你的要求选店、选商品和真实规格，并创建待付款订单；不会展示或模拟角色钱包、自动扣款。</div><div class="delivery-section-title">我的外卖偏好</div>'+prefRows+'<button class="delivery-role-row" onclick="deliveryOpenSavedRoutes()"><span>📍</span><span><b>常用直达路线</b><small>查看角色能真实打开的门店和商品；角色不会全网自动搜索</small></span><i>›</i></button><div class="delivery-field"><label>真实服务状态</label><div><input value="'+esc(capText)+'" disabled><button onclick="deliveryRefreshCapabilities(true)" '+(!r.enabled?'disabled':'')+'>重新检测</button></div></div><div class="delivery-field"><label>当前收货地址</label><div><input value="'+esc(r.addressLabel||'尚未确认')+'" disabled><button onclick="deliveryConfirmAddress()" '+(!r.enabled||!connected||(cap&&cap.addressConfirmation===false)?'disabled':'')+'>本人确认</button></div><small>'+(cap&&cap.addressConfirmation===false?'当前真实外卖服务不支持地址确认':r.approvedAddressFingerprint?'已确认当前平台默认地址':'连接成功后，请在创建订单前确认平台默认地址')+'</small></div><div class="delivery-safety">角色只使用已登记的常用直达路线，最多核对三家同品牌同商品门店；门店打烊会换下一家，全部不可用时会停止并自然告诉你。你本人手动搜索新商品时仍可能遇到平台验证；支付密码和生物识别始终由你本人完成。</div></div>');
  }
  ...
  async function realSearch(query,opt){
    ...
    var r=foodState(),data=await request('search',{query:query,providers:['taobao_flash'],paymentPreference:['alipay'],roleId:opt&&opt.roleId||'',addressLabel:r.addressLabel||'',addressFingerprint:r.approvedAddressFingerprint||'',limit:opt&&opt.roleId?3:4},165000);
  ...
  async function chooseOffer(c,query,offers){
    ...
    var raw=await chatAPI([{role:'system',content:'你是「'+c.name+'」本人，只做淘宝闪购候选选择，不要对用户说话。用户本次明确说出的品牌/店名和具体商品是硬条件：候选没有完全对应项时必须返回 matched:false，绝不能用相近品牌、别家门店或别的商品替代。用户没有明确指定的部分，才参考全部长期外卖偏好、你的人设、真实价格、评价、优惠和配送信息。只返回严格 JSON：{"matched":true,"offerId":"候选id","reason":""}；不能满足时返回 {"matched":false,"offerId":"","reason":"缺少的真实品牌或商品"}。只能使用候选真实 id。'},{role:'user',content:'用户本次原话：'+query+'\n全部长期外卖偏好（本次要求优先）：\n'+preferenceText()+'\n淘宝闪购真实候选：'+JSON.stringify(compact)}],{max:260,temp:.1});
  ...
  async function chooseOptions(c,query,offer){
    ...
    var raw=await chatAPI([{role:'system',content:'你是「'+c.name+'」本人，只做淘宝闪购真实规格选择，不要对用户说话。用户本次明确说出的杯型、份量、糖度、冰度、温度、口味、辣度、搭配和加料都是硬条件；真实选项缺少任意一项时必须返回 matched:false，绝不能擅自改规格或用相近选项替代。用户没说清的部分才参考全部长期外卖偏好和你的人设。不喜欢、忌口和过敏项必须避开。每个 required 组必须选择，只能使用真实 id。只返回严格 JSON：{"matched":true,"quantity":1,"selectedOptions":{"组选项id":"选择id"},"reason":""}；多选组用数组；不能满足时返回 {"matched":false,"quantity":1,"selectedOptions":{},"reason":"缺少的规格"}。'},{role:'user',content:'用户本次原话：'+query+'\n全部长期外卖偏好（本次要求优先）：\n'+preferenceText()+'\n已严格匹配的真实商品：'+offer.merchant+' / '+offer.name+'\n平台真实规格：'+JSON.stringify(groups)}],{max:380,temp:.1});
  ...
  function roleSystemNotice(message,kind){var r=foodState(),msg=text(message,180);r.lastRoleNotice={message:msg,kind:text(kind,20)||'info',at:Date.now()};if(kind==='error')r.lastError=msg;save();if(msg)toast(msg);}
  ...
  async function roleRequest(cid,query){
    ...
    catch(e){attempt.status='failed';attempt.endedAt=Date.now();attempt.error=text(e.message||'真实外卖操作失败',180);attempt.failureKind=/打烊|休息中|门店休息|暂停营业|不在营业时间/.test(attempt.error)?'shop_closed':'';save();resultReply(c,order,attempt.error,attempt.failureKind);return true;}
  ...
  function rolePrompt(c){
    ...
    return '\n\n# 真实外卖能力与硬边界\n真实外卖已开启，但你始终保留自己的判断和意愿：可以按你的人设、关系、情绪与当下处境同意，也有权直接拒绝给'+S.me.name+'点单。拒绝时只需用你自己的口吻正常说明，不要输出任何外卖标签，也不会启动真实订单。你可以按本人判断主动使用，也可以在'+S.me.name+'明确让你点某样东西时搜索。若对方只是泛泛说“想喝奶茶”“想吃点东西”，还没说明具体品牌/门店或具体商品，也没有明确让你自行决定，就必须先按你自己的语气自然问清楚并等待回答；这一轮绝不能输出真实外卖标签。若对方明确让你自行决定，你要结合奶茶、主食、咖啡和 KFC 的全部已保存偏好、你的人设、关系和当下处境，自主选择类别、品牌与具体商品；偏好只补足对方没有说清的部分，不能覆盖本轮明确要求。只有你自己同意，并且对方已经说清品牌/门店与商品，或者在你询问后明确让你自行决定，才可以开始真实搜索。\n真正决定尝试时，必须先发一句符合你本人语气的简短可见消息，让'+S.me.name+'知道你正在找，例如自然表达“等我一下”“我去看看”；紧接着再单独输出一行 [真实外卖|具体搜索词]。搜索词要包含明确品牌/门店和具体商品，并保留用户说出的糖度、冰度、温度、口味、辣度、份量或加料。系统只会在本人已经登记的同品牌同商品直达路线中最多核对三家，重新读取营业状态、商品、价格和规格；绝不会由角色发起全网自动搜索。第一家打烊会试下一家，全部打烊时系统会把真实事实交给你，让你自然告诉用户。严格匹配后才创建支付宝待付款订单，由用户本人付款。\n开始操作的可见消息只能表达“准备找/准备看看”，绝不能提前声称已经找到、用了优惠券、下单、付款、骑手接单或送达；只有收到后续真实操作结果后才能说对应事实。找不到准确品牌、商品或规格时必须如实失败，不能换品牌、换商品或改口味，也不能改走虚拟外卖。不要再使用 [点外卖|餐品|价格]。';
  }

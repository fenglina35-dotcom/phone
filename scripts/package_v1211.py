...
def validate(web,private):
    ...
    for token in ["APP_VER='v1211",'modelOutputUnfilteredToggle','lifeNoteCommitReply','roleSocialIdentityPin','proactiveContinuationContext']:
    ...
    assert 'private-smart-lock.js?v=336' in pshell and 'private-smart-lock.css?v=336' in pshell
    ...
    for name in ['message-beijing-time.js','cohab-theater.js','pixel-home.js','pixel-home-policy.js','pixel-wardrobe-info.js']:
    ...
    assert project.count('CURRENT_PROJECT_VERSION = 336;')==12
    assert project.count('MARKETING_VERSION = 1.0.336;')==12
    assert 'static let contractVersion = 36' in private['PhoneCompanionTest/PhoneNativeBridge.swift'].decode('utf8')
    assert "const BUILD='1211'" in web['sw.js'].decode('utf8')
    assert "__NORTH_SHELL_BUILD__='1211'" in shell and "__NORTH_SHELL_BUILD__='1211'" in pshell
    ...
    assert 'message-beijing-time.js?v=1211' in shell and 'message-beijing-time.js?v=1211' in pshell
    assert all('phone-shortcuts' not in n for n in list(web)+list(private))
def main():
    ...
    if not args.check:
        ...
        allowed={'supabase/functions/phone-shortcuts/core.ts','supabase/migrations/202609080002_phone_shortcuts.sql'}

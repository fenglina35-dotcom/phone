...
function runtime(){
  ...
  const sandbox=vm.createContext({console,JSON,Math,Date,String,Array,Number,Object,Set,Map,Promise,
    ...
    gameLineIcon:()=>'',esc:s=>String(s),closeModal(){},openModal(){},go(){},openGames(){},uiConfirm:async()=>true,chatAPI:async()=>'',buildSystem:()=>'',gameModelUseAux:()=>false,roleChatRouteIndex:()=>0,cleanReply:s=>String(s),parseObj:JSON.parse,gameSetHandoff(){},stack:[{p:'home'},{p:'gameshub'},{p:'beadstudio'}],NORTH_PREVIEW_PARAMS:''
  });
  ...
  sandbox.__out.__sandbox=sandbox;
...
test('game hall routes the shared Bead Atelier in web and private runtimes',()=>{
  ...
  assert.match(html,/bead-studio\.js\?v=1179&r=v1179-pixel-puzzle-2/);
  assert.match(privateHtml,/bead-studio\.js\?v=1179&r=v1179-pixel-puzzle-2/);
  assert.match(sw,/bead-studio\.js\?v='\+BUILD\+'\&r=v1179-pixel-puzzle-2/);
...
test('uploaded images are converted into square color-code charts up to 128 cells',()=>{
  ...
  assert.match(bead,/<div><b>像素拼拼乐<\/b><\/div><span>/,'the game name must occupy the fixed top-center slot');
  ...
  assert.match(bead,/g\.completionMessaged=true;beadSave\(g\)/,'completion follow-up is scheduled at most once after durable save');
  assert.match(bead,/typeof scheduleReply==='function'/,'the genuine role pipeline sends the post-completion WeChat message');
  assert.match(bead,/保存进度/,'unfinished work has an explicit save-without-exit action');
  assert.match(bead,/保存退出/,'unfinished work has an explicit save-and-exit action');

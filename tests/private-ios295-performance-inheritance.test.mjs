...
test('private identifiers advance to v1237 and iOS 355 while public stays v1237', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1237'/);
  assert.match(index, /app\.js\?v=1237&r=v1237-backup-streaming-1/);
  ...
  assert.match(publicApp, /APP_VER='v1237 · 大备份分段读写'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1237'/);
...
test('Mac guide distinguishes v1237 source sync from the unchanged iOS355 package', () => {
  ...
  assert.match(mac, /^# v1237 .*仅同步内置网页源码/);

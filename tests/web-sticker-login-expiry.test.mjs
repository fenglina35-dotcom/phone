...
test('the web shell and service worker require the hotfix while private files stay outside its entry path',()=>{
  assert.match(shell,/web-hotfix\.js\?v=1123&r=wardrobe-time-range-1/);
  assert.match(hotfix,/sw\.js\?v=1123&r=v1123-wardrobe-time-range-1/);
  assert.match(worker,/kind:'hotfix'/);
  assert.match(worker,/north-shell-v1123-wardrobe-time-range-1/);

...
DELIVERY = ROOT / "delivery-v1032"
PACKAGE_NAME = "SmallPhone_v1032_WeChatChat_iOS153_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百五十三次安装_v1032_微信聊天视觉升级_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1032":
...
if "APP_VER='v1032 · 微信聊天视觉升级';" not in app_text:
    raise RuntimeError("bundled web version is not v1032")
project_text = (STAGING / "PhoneCompanionTest.xcodeproj" / "project.pbxproj").read_text(
    encoding="utf-8"
)
if project_text.count("CURRENT_PROJECT_VERSION = 153;") != 12:
    raise RuntimeError("private iOS build number is not consistently 153")
if project_text.count("MARKETING_VERSION = 1.0.153;") != 12:
    raise RuntimeError("private iOS marketing version is not consistently 1.0.153")

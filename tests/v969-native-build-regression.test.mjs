...
test('public web v1184 and private web v1189 remain compatible with native iOS 1.0.315', () => {
  ...
  assert.match(app, /sw\.js\?v=1196&r=v1196-couple-watch-trigger-1/);
  assert.match(shell, /north-shell-v1196-couple-watch-trigger-1/);
  assert.match(bundleInfo, /<string>1196<\/string>/);
  assert.match(localWebView, /1\.0\.318 \(318\)/);
  assert.equal((project.match(/CURRENT_PROJECT_VERSION = 318;/g) || []).length, 12);
  assert.equal((project.match(/MARKETING_VERSION = 1\.0\.318;/g) || []).length, 12);
});

...
test('private identifiers remain v1303 and iOS 385 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1303'/);
  assert.match(index, /app\.js\?v=1303&r=v1303-private-imsg-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=339/);
  assert.match(app, /APP_VER='v1303 · 抖音发作品修复、文字特效、隐形墨水刮开（私人）'/);
  assert.match(overlay, /336-daily-file-backup/);
  assert.match(webview, /1\.0\.385 \(385\)/);
  assert.match(bridge, /private static let build = "1\.0\.385 \(385\)"/);
  assert.match(bridge, /static let contractVersion = 41/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 385;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.385;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1302 · 抖音发作品修复、文字特效、隐形墨水刮开'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1302'/);
...
test('Mac guide identifies the current private v1303 iOS385 source', () => {

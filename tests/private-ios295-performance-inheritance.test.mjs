...
test('v1189 private identifiers retain the performance chain while public stays v1184', () => {
  ...
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1200'/);
  assert.match(index, /app\.js\?v=1200&r=v1200-couple-watch-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=322/);
  assert.match(app, /APP_VER='v1200 · 回复接力互斥版'/);
  assert.match(overlay, /322-handoff-ownership-1/);
  assert.match(webview, /1\.0\.322 \(322\)/);
  assert.match(bridge, /private static let build = "1\.0\.322 \(322\)"/);
  assert.match(bridge, /static let contractVersion = 35/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 322;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.322;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1200 · 回复接力互斥版'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1200'/);
...
test('Mac guides state the private v1189 and unchanged public web v1184 boundary', () => {
  ...
  assert.match(mac, /当前候选：网页源码与私人内置网页 v1200/);
  assert.match(mac, /网页.*推送/);
  assert.match(install, /网页源码与私人内置网页均为 v1200/);

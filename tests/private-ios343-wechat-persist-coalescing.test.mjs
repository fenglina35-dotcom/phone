...
test('private identity advances to v1268 and iOS 368 while public remains v1268', () => {
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!=='1268'/);
  assert.match(privateApp, /APP_VER='v1268 · 抖音群聊与火花'/);
  for (const html of [privateIndex, privateAlias]) {
    assert.match(html, /window\.__NORTH_SHELL_BUILD__='1268'/);
    assert.match(html, /app\.js\?v=1268&r=v1268-private-sync-1/);
    assert.match(html, /private-runtime-diagnostics\.js\?v=336/);
  }
  assert.match(privateRepair, /index\.html\?repair=1&v=1268/);
  assert.match(swift, /1\.0\.368 \(368\)/);
  assert.match(bridge, /private static let build = "1\.0\.368 \(368\)"/);
  assert.equal((project.match(/CURRENT_PROJECT_VERSION = 368;/g) || []).length, 12);
  assert.equal((project.match(/MARKETING_VERSION = 1.0.368;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1268 · 抖音群聊与火花'/);
  assert.match(publicIndex, /window\.__NORTH_SHELL_BUILD__='1268'/);

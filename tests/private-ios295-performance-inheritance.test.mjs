...
test('private identifiers advance to v1227 and iOS 350 while public stays v1226', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1227'/);
  assert.match(index, /app\.js\?v=1227&r=v1227-private-combined-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=334/);
  assert.match(app, /APP_VER='v1227 · 私人明确语音请求修复版'/);
  ...
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 350;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.350;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1226 · 明确语音请求修复版'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1226'/);
...
test('packaged Mac guide advances to v1227 iOS 350', () => {

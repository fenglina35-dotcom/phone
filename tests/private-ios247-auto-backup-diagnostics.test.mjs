...
test('private iOS 298 overlay owns the disable marker and cancels automatic scheduling', async () => {
  const { context, calls } = makeAutoBackupHarness();
  assert.equal(
    context.__SMALL_PHONE_PRIVATE_RUNTIME__,
    '298-cohab-theater-v1'
  );
...
test('manual backup and both restore actions remain free of the automatic-disable guard', () => {
  for (const name of [
    ...
    assert.doesNotMatch(source, /298-cohab-theater-v1/);
  }
  assert.doesNotMatch(
    overlay,
...
test('native recovery UI stays outside WebKit and carries the private 298 identity', () => {

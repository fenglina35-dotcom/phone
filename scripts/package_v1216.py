...
def validate(web,private):
    ...
    assert "APP_VER='v1216" in app and "APP_VER='v1216" in papp
    ...
    assert 'request-diagnostics.js?v=1216' in shell and 'request-diagnostics.js?v=1216' in pshell
    ...
    assert project.count('CURRENT_PROJECT_VERSION = 341;')==12
    assert project.count('MARKETING_VERSION = 1.0.341;')==12
    assert 'static let contractVersion = 36' in private['PhoneCompanionTest/PhoneNativeBridge.swift'].decode('utf8')
    assert "const BUILD='1216'" in web['sw.js'].decode('utf8')
    assert "__NORTH_SHELL_BUILD__='1216'" in shell and "__NORTH_SHELL_BUILD__='1216'" in pshell
    assert GUIDE in private
    assert 'native-management-batch' in private['PhoneCompanionTest/CompanionSyncView.swift'].decode('utf8')
    assert 'by: by' in private['PhoneCompanionTest/PhoneNativeBridge.swift'].decode('utf8')
    assert "by:opt&&opt.by==='role'?'role':'owner'" in papp
    for name in ['delivery.js','web-hotfix.js']:
        assert web[name]==private[BUNDLE+name],name
    assert 'privateBackgroundModelErrorGuard:true' in papp and 'privateBackgroundModelErrorGuard' not in app
    assert 'message-beijing-time.js?v=1216' in shell and 'message-beijing-time.js?v=1216' in pshell
    for name in ['public-north-policy.js','public-north-runtime.js','phone-shortcuts.js']:
        assert web[name]==private[BUNDLE+name],name
        assert name+'?v=1216' in shell and name+'?v=1216' in pshell
    assert 'NorthPublicRuntime' in app and 'NorthPublicRuntime' in papp
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--check',action='store_true');parser.add_argument('--allow-untracked',action='append',default=[]);parser.add_argument('--allow-dirty',action='append',default=[]);args=parser.parse_args()
    assert git('branch','--show-current').decode().strip()=='main'
    if not args.check:
        dirty=set(git('diff','HEAD','--name-only','-z').decode('utf8').strip('\0').split('\0'))-{''}
        assert dirty<=set(args.allow_dirty),f'Uncommitted selected files: {dirty-set(args.allow_dirty)}'

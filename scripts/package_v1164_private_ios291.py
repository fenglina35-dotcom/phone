...
INSTALL_GUIDE_SOURCE = (
    SOURCE / "第二百九十一次安装_v1164_列表视频与智能家电修复_请先读.md"
)
DELIVERY = ROOT / "delivery-v1164-private291-scroll-camera-smart-home-candidate"
PACKAGE_NAME = "SmallPhone_v1164_ScrollCameraSmartHome_iOS291_MacReady"
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
USER_ZIP = ROOT.parent / "小手机_v1164_私人版_iOS291_列表视频与智能家电修复_Mac待编译源码包.zip"
EXPECTED_BUNDLE_FILES = 148
EXPECTED_PACKAGE_FILES = 187
...
if tracked_status and not ALLOW_DIRTY_PACKAGE:
    raise RuntimeError("tracked working tree is dirty; commit private iOS 291 first")
...
with tempfile.TemporaryDirectory(prefix="smallphone-v1164-ios291-", dir=ROOT) as temp:
    ...
    for token in [
        "window.__NORTH_SHELL_BUILD__='1164'",
        'private-runtime-diagnostics.js?v=291',
        "app.js?v=1164",
    ]:
    ...
    if "index.html?repair=1&v=1164" not in repair:
        raise RuntimeError("private repair page does not return to v1164")
    for destructive in [
        "localStorage.clear",
    ...
    for token in [
        "291-scroll-camera-smart-home-v1",
        "recovery.launch.peek",
        "recovery.launch.ack",
        "window.emergencyRestoreAll()",
    ]:
        if token not in overlay:
            raise RuntimeError(f"private 291 recovery marker missing: {token}")

    project = (
        staging / "PhoneCompanionTest.xcodeproj/project.pbxproj"
    ).read_text(encoding="utf-8")
    if project.count("CURRENT_PROJECT_VERSION = 291;") != 12:
        raise RuntimeError("private build 291 is not set on all targets/configurations")
    if project.count("MARKETING_VERSION = 1.0.291;") != 12:
        raise RuntimeError("private version 1.0.291 is not set consistently")
    ...
    for token in [
        "1.0.291 (291)",
        "smallPhone.webContentTerminationTimes.v5.build291",
        "native.webview.make",
    ...
    if "私人 iOS 1.0.291 (291)" not in main_guide:
        raise RuntimeError("Mac guide does not identify private iOS 291")

...
test('private identifiers remain v1274 and iOS 373 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1274'/);
  assert.match(index, /app\.js\?v=1274&r=v1274-private-sync-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=339/);
  assert.match(app, /APP_VER='v1274 · 私人版温馨小家材质稳定修复'/);
  assert.match(overlay, /336-daily-file-backup/);
  assert.match(webview, /1\.0\.373 \(373\)/);
  assert.match(bridge, /private static let build = "1\.0\.373 \(373\)"/);
  assert.match(bridge, /static let contractVersion = 38/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 373;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.373;/g) || []).length, 12);
...
test('Mac guide identifies the current private v1274 iOS373 source', () => {

...
INSTALL_GUIDE_SOURCE = (
    SOURCE / "第二百九十三次安装_v1166_心动审判分批出题修复_请先读.md"
)
DELIVERY = ROOT / "delivery-v1166-private293-heartquiz-batches-candidate"
PACKAGE_NAME = "SmallPhone_v1166_HeartQuizBatches_iOS293_MacReady"
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
USER_ZIP = ROOT.parent / "小手机_v1166_私人版_iOS293_心动审判分批出题修复_Mac待编译源码包.zip"
...
if tracked_status and not ALLOW_DIRTY_PACKAGE:
    raise RuntimeError("tracked working tree is dirty; commit private iOS 293 first")
...
with tempfile.TemporaryDirectory(prefix="smallphone-v1166-ios293-", dir=ROOT) as temp:
    ...
    for token in [
        "window.__NORTH_SHELL_BUILD__='1166'",
        'private-runtime-diagnostics.js?v=293',
        "app.js?v=1166",
    ]:
    ...
    if "index.html?repair=1&v=1166" not in repair:
        raise RuntimeError("private repair page does not return to v1166")
    for destructive in [
        "localStorage.clear",
    ...
    for stale in [
        "max:9000",
        "complete:true,roleInterceptAudit:null,roleInterceptStage:'心动审判出题'",
        "必须正好30项",
    ]:
        if stale in heart_quiz:
            raise RuntimeError(f"private heart-quiz still contains one-shot generation: {stale}")

    overlay = (bundle / "private-runtime-diagnostics.js").read_text(encoding="utf-8")
    for token in [
        "293-heartquiz-batches-v1",
        "recovery.launch.peek",
        "recovery.launch.ack",
        "window.emergencyRestoreAll()",
    ]:
        if token not in overlay:
            raise RuntimeError(f"private 293 recovery marker missing: {token}")

    project = (
        staging / "PhoneCompanionTest.xcodeproj/project.pbxproj"
    ).read_text(encoding="utf-8")
    if project.count("CURRENT_PROJECT_VERSION = 293;") != 12:
        raise RuntimeError("private build 293 is not set on all targets/configurations")
    if project.count("MARKETING_VERSION = 1.0.293;") != 12:
        raise RuntimeError("private version 1.0.293 is not set consistently")
    ...
    for token in [
        "1.0.293 (293)",
        "smallPhone.webContentTerminationTimes.v5.build293",
        "native.webview.make",
    ...
    for token in [
        "contractVersion = 26",
        'private static let build = "1.0.293 (293)"',
    ...
    if "私人 iOS 1.0.293 (293)" not in main_guide:
        raise RuntimeError("Mac guide does not identify private iOS 293")

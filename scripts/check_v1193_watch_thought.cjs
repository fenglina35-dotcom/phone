...
(async()=>{
 ...
 try{
  await Promise.all(['chat','douyin'].map(async kind=>{
   ...
   console.log(JSON.stringify({kind,elapsed,modelRequests:1,delivered:4,canonicalThought:true,thoughtSurvivesReopen:true,version:result.version,pageErrors:errors}));
   await context.close();
  }));

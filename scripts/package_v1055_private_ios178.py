...
DELIVERY = ROOT / "delivery-v1055-private178-final"
PACKAGE_NAME = "SmallPhone_v1055_PrivateComplete_iOS178_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百七十八次安装_v1055_快餐套餐与去重稳定_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1055-private178-final":
...
for token in [
    ...
    if token not in delivery_text:
        raise RuntimeError(f"v1055 bundled delivery feature missing: {token}")
...
if "window.__NORTH_SHELL_BUILD__='1055'" not in shell_text or "delivery.js?v=1055" not in shell_text:
    raise RuntimeError("private shell is not v1055")
if "<string>1055</string>" not in (bundle / "Info.plist").read_text(encoding="utf-8"):
    raise RuntimeError("PhoneWeb.bundle Info.plist is not v1055")

project_text = (STAGING / "PhoneCompanionTest.xcodeproj/project.pbxproj").read_text(encoding="utf-8")
if project_text.count("CURRENT_PROJECT_VERSION = 178;") != 12 or project_text.count("MARKETING_VERSION = 1.0.178;") != 12:
    raise RuntimeError("private iOS version is not consistently 1.0.178 (178)")
webview_text = (STAGING / "PhoneCompanionTest/LocalPhoneWebView.swift").read_text(encoding="utf-8")
if "__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.178 (178)'" not in webview_text:
    raise RuntimeError("private build marker is not 1.0.178 (178)")

...
test('time facts include short exact gaps and generated claims are verified once',()=>{
  ...
  assert.match(app,/function roleReplyRequestPin\(c,now\)[\s\S]*?personaPin\(c\)\+roleReplyClockPin\(now\)\+roleReplyTimelinePin\(c\)\+roleReplyContinuityPin\(c,now\)\+roleReplyCrossChannelHandoffPrompt\(c,now\)/);
...
test('active memory frequency is a meaningful daily ceiling',()=>{
  ...
  assert.match(app,/function rememberFromConversation\(c,text,userText,replyText,opt\)/);

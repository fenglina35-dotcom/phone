...
DELIVERY = ROOT / "delivery-v1041"
PACKAGE_NAME = "SmallPhone_v1041_QuoteVideoGame_iOS160_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百六十次安装_v1041_微信引用视频兼容与游戏返回修复_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1041":
...
if "APP_VER='v1041 · 微信引用、视频兼容与游戏返回修复版';" not in app_text:
    raise RuntimeError("bundled web version is not v1041")
for required_text in [
    ...
    if required_text not in app_text:
        raise RuntimeError(f"v1041 bundled repair is missing: {required_text}")
...
for required_text in [
    ...
    if required_text not in theme_text:
        raise RuntimeError(f"v1041 quote layout is missing: {required_text}")

info_text = (bundle / "Info.plist").read_text(encoding="utf-8")
if "<string>1041</string>" not in info_text:
    raise RuntimeError("PhoneWeb.bundle Info.plist is not v1041")
...
if project_text.count("CURRENT_PROJECT_VERSION = 160;") != 12:
    raise RuntimeError("private iOS build number is not consistently 160")
if project_text.count("MARKETING_VERSION = 1.0.160;") != 12:
    raise RuntimeError("private iOS marketing version is not consistently 1.0.160")

webview_text = (
    STAGING / "PhoneCompanionTest" / "LocalPhoneWebView.swift"
).read_text(encoding="utf-8")
if "__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.160 (160)'" not in webview_text:
    raise RuntimeError("private build marker is not 1.0.160 (160)")

...
test('private performance candidate keeps v1277 while public web advances independently', () => {
  assert.equal(privateIndex, privateAlias);
  assert.match(privateIndex, /window\.__NORTH_SHELL_BUILD__='1277'/);
  assert.match(privateIndex, /app\.js\?v=1277&r=v1277-private-backup-progress-1/);
  assert.match(privateIndex, /private-runtime-diagnostics\.js\?v=339/);
  assert.match(privateRepair, /index\.html\?repair=1&v=1277/);

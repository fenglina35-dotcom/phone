...
test('private identifiers advance to v1224 and iOS 347 while public stays v1223', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1224'/);
  assert.match(index, /app\.js\?v=1224&r=v1224-private-ac-temperature-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=334/);
  assert.match(app, /APP_VER='v1224 · 私人空调温度修复版'/);
  assert.match(overlay, /334-wechat-persist-lane/);
  assert.match(webview, /1\.0\.347 \(347\)/);
  assert.match(bridge, /private static let build = "1\.0\.347 \(347\)"/);
  assert.match(bridge, /static let contractVersion = 37/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 347;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.347;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1223 · 聊天重新生成修复版'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1223'/);
...
test('current Mac guide advances to private v1224 and iOS 347 while the historical iOS 327 record remains historical', () => {
  ...
  for (const guide of [install, mac]) {
    assert.match(guide, guide===install?/v1206/:/v1213/);

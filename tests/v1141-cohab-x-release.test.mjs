...
test('v1181 web identity and cache markers are aligned', () => {
  ...
  assert.match(sw, /v1181-theater-presence-release-1/);
  assert.match(hotfix, /v1181-theater-presence-release-1/);
  assert.match(index, /小手机\.html\?v=1181/);
  assert.match(repair, /小手机\.html\?v=1181/);
});

test('v1181 publishes shared cohab memory and X comment controls', () => {
  const app = read('app.js');

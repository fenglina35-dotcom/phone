...
test('private identity advances to v1254 and iOS 362 while public remains v1253', () => {
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!=='1254'/);
  ...
  for (const html of [privateIndex, privateAlias]) {
    ...
    assert.match(html, /app\.js\?v=1254&r=v1254-private-sync-1/);
    assert.match(html, /private-runtime-diagnostics\.js\?v=336/);
  }
  assert.match(privateRepair, /index\.html\?repair=1&v=1254/);
  assert.match(swift, /1\.0\.362 \(362\)/);
  ...
  assert.match(publicIndex, /window\.__NORTH_SHELL_BUILD__='1253'/);

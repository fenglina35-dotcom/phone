"""Create the private v1331 / iOS 399 Mac-source overlay package.
...
PREFIX = "SmallPhone_v1331_iOS399_Private/"
OUTPUT = ROOT.parent / "SmallPhone_v1331_iOS399_MacSource.zip"
...
def validate(files: dict[str, bytes]) -> None:
    ...
    for marker, label in [
        ("function explicitMemoryFromUserText(", "v1331 明确长期外卖偏好直接保存"),
        ("function contextualKfcAction(", "v1331 KFC 随便点和四件套续接"),
        ("function brandOnlyDeliveryRequest(", "v1331 只说品牌时自然追问"),
    ]:
...
def main() -> None:
    ...
    with TemporaryDirectory(prefix="private1331-verify-") as folder:
        with ZipFile(OUTPUT) as archive:

...
test('v1178 shared theater asset is byte-identical and loaded after app core',()=>{
  ...
  assert.match(webHtml,/app\.js\?v=1259[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1259&r=v1184-ios-web-crash-cohab-turn-keyboard-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1258[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1258[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1258&r=v1184-ios-web-crash-cohab-turn-keyboard-1"/);
...
test('private artifact identity advances independently to v1258 and iOS 1.0.364 (364)',()=>{
  assert.match(privateApp,/const APP_VER='v1258 · 私人同步 App 管控修复'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=336/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.364 \(364\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 364;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.364;/g)||[]).length>=12);
});

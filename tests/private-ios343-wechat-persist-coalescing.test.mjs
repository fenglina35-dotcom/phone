...
test('private identity remains v1327 and iOS 394 while public advances independently', () => {
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!=='1327'/);
  ...
  for (const html of [privateIndex, privateAlias]) {
    ...
    assert.match(html, /app\.js\?v=1327&r=v1327-private-delivery-tag-1/);
    assert.match(html, /private-runtime-diagnostics\.js\?v=339/);
  }
  assert.match(privateRepair, /index\.html\?repair=1&v=1327/);
  assert.match(swift, /1\.0\.397 \(397\)/);
  ...
  assert.match(publicIndex, /window\.__NORTH_SHELL_BUILD__='1326'/);

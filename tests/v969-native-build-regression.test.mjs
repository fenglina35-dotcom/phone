...
test('v1023 web and private iOS 1.0.144 keep explicit build identities', () => {
  assert.match(app, /APP_VER='v1023 · 伴生快照写入放大与发热修复'/);
  assert.match(app, /sw\.js\?v=1023&r=v1023-companion-snapshot-write-repair-1/);
  assert.match(shell, /north-shell-v1023/);
  assert.match(bundleInfo, /<string>1023<\/string>/);
  assert.match(localWebView, /1\.0\.144 \(144\)/);

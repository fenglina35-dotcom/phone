...
function renderCozyHome(){const i=cozyIdentity();if(!i)return '<div class="nav"><button class="l" onclick="back()">‹</button><span>请先在情侣空间绑定角色</span></div>';cozyClose();const token=Array.from(crypto.getRandomValues(new Uint32Array(4)),n=>n.toString(16)).join('');const native=location.protocol==='file:'&&privateNativeAppOn(),url=native?'cozy-home://app/index.html':'games/cozy-home/index.html';_cozyHome={...i,token,origin:native?'null':location.origin,busy:false,micEnabled:false,seen:new Set(),scene:null};return '<iframe id="cozy-home-frame" title="小家" allow="autoplay; microphone; fullscreen" style="position:absolute;inset:0;width:100%;height:100%;border:0;background:#ede4d7" src="'+url+'?private=1&amp;build=1273&amp;session='+token+'"></iframe>';}

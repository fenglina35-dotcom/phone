...
test('private build and bundled recovery page advance together', () => {
  assert.match(webView, /__SMALL_PHONE_PRIVATE_BUILD__ = '1\.0\.316 \(316\)'/);
  assert.match(webView, /smallPhone\.webContentTerminationTimes\.v25\.build316/);
  assert.match(privateApp, /APP_VER='v1190 · 情侣空间聊天与软件监管版'/);
  assert.equal(privateAlias, privateIndex);
  assert.match(privateIndex, /window\.__NORTH_SHELL_BUILD__='1190'/);
  assert.match(privateIndex, /app\.js\?v=1190/);
  assert.match(privateIndex, /private-runtime-diagnostics\.js\?v=316/);
  assert.match(privateRepair, /index\.html\?repair=1&v=1190/);
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!==\'1190\'/);
});

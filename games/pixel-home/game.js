...
(async () => {
  ...
  function dollMetrics(){let bottom=Math.min(H*.94,H-95),h=Math.min(H*.80,W*1.48,Math.max(170,bottom-105)),x=W*.50;if(drawer){bottom=H-$('#drawer').offsetHeight-12;h=Math.min(H*.40,W*.70,Math.max(65,bottom-65));}if(state.sleeping&&!drawer){h=Math.min(H*.40,W*.76);x=W*.58;bottom=H*.72;}const bp=window.RoseWardrobe?.getState().body||{size:100,legs:85},size=bp.size/100,offset=490*(1-bp.legs/100),renderY=bottom-h,renderH=h;return{h:h*size,x,y:renderY+h/1536*(1450*(1-size)+offset*size),s:h*size/512,w:h*size*320/512,renderY,renderH,footY:renderY+h*1450/1536};}
  function drawDoll(g,x,y,h,opts={}){
    if(window.RoseWardrobe){window.RoseWardrobe.draw(g,x,y,h,{...opts,closed:opts.closed??(now<blinkUntil||['pet','eat','wash','hug'].includes(action))});return;}
    const s=h/512;
    ...
    g.drawImage(ASSETS.longEspresso,192,closed?512:0,320,512,-160*s,0,320*s,h);
    g.restore();
  }
  function sceneCamera(){}
  function drawBackground(){ctx.save();sceneCamera();if(state.room===0){ctx.drawImage(ASSETS.bedroom,0,0,W,H);}else{const im=ASSETS.rooms;ctx.drawImage(im,(state.room-1)*im.width/2,0,im.width/2,im.height,0,0,W,H);} ctx.restore(); // Room image and sleeping overlay share the same camera.
  ...
  function draw(t){raf=0;if(document.hidden||!ready)return;if(t-lastFrame<32){raf=requestAnimationFrame(draw);return;}const dt=Math.min(t-lastFrame||32,80);lastFrame=t;now=t;ctx.setTransform(dpr,0,0,dpr,0,0);ctx.imageSmoothingEnabled=false;ctx.clearRect(0,0,W,H);drawBackground();if(now>actionUntil){action='idle';reactX=0;}if(now>nextBlink){blinkUntil=now+140;nextBlink=now+2400+Math.random()*3000;}
    const m=dollMetrics();if(mirrorMode&&!drawer){drawMirror();}else if(state.sleeping&&!drawer){ctx.save();sceneCamera();ctx.drawImage(ASSETS.sleepP80,0,0,W,H);ctx.drawImage(ASSETS.sleepFootboard,0,0,W,H);ctx.restore();ctx.fillStyle='#fff6e9';ctx.font='16px Georgia';ctx.fillText('z',W*.89,H*.37+Math.sin(now/1000)*3);ctx.font='10px Georgia';ctx.fillText('z',W*.92,H*.345+Math.sin(now/1000)*3);}else{ctx.save();ctx.fillStyle='#67402722';ctx.beginPath();ctx.ellipse(m.x,m.footY,m.w*.30,8,0,0,Math.PI*2);ctx.fill();ctx.restore();drawDoll(ctx,m.x,m.renderY,m.renderH);}
  ...
  function openPanel(name){if(!ready)return;if(name==='wardrobe'||name==='vanity'){cancelCare(false);cancelPointers();window.PixelHomeBridge.request('wardrobe-edit',{editing:true}).catch(()=>{});save();const panel=$('#wardrobe-overlay');panel.hidden=false;$('#wardrobe-frame').src='wardrobe/index.html?scope='+encodeURIComponent(initial.scope);return;}advance();setMirror(false,true);cancelCare(false);cancelPointers();ball=null;selected=null;drawer=name;document.body.classList.add('drawer-open');tab=name==='vanity'?'hair':'sets';$('#drawer').hidden=false;renderPanel();document.querySelectorAll('[data-panel]').forEach(b=>b.classList.toggle('active',b.dataset.panel===name));}
  ...
  async function takePhoto(){
    ...
    try{
      ...
      state.photos.push({image,label:new Date().toLocaleDateString('zh-CN')+' · '+(window.RoseWardrobe?.catalog.items[window.RoseWardrobe.getState().dress]?.name||'今日穿搭')});state.photos=state.photos.slice(-6);
  ...
  function drawMirror(){
    ...
    drawDoll(ctx,m.x,m.iy,512*m.s,{still:true,portrait:true,closed:now<blinkUntil||action==='wash'});

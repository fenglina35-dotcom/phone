...
def validate(web,private):
    ...
    for token in ["APP_VER='v1210",'modelOutputUnfilteredToggle','lifeNoteCommitReply','roleSocialIdentityPin','proactiveContinuationContext']:
    ...
    assert project.count('CURRENT_PROJECT_VERSION = 334;')==12
    assert project.count('MARKETING_VERSION = 1.0.334;')==12
    assert 'static let contractVersion = 36' in private['PhoneCompanionTest/PhoneNativeBridge.swift'].decode('utf8')
    assert "const BUILD='1210'" in web['sw.js'].decode('utf8')
    assert "__NORTH_SHELL_BUILD__='1210'" in shell and "__NORTH_SHELL_BUILD__='1210'" in pshell
...
def main():
    ...
    results=[]
    for kind,files,title in [('网页',web,'SmallPhone_v1210_Web'),('私人',private,'SmallPhone_v1210_iOS334_MacSource')]:
        if kind=='私人':files={p:b for p,b in files.items() if not (len(PurePosixPath(p).parts)==1 and '安装' in p and p!=GUIDE)}
        name='小手机_v1210_网页版_聊天中断紧急修复.zip' if kind=='网页' else '小手机_v1210_私人版_iOS334_聊天中断紧急修复_Mac待编译源码包.zip'

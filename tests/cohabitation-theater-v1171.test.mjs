...
test('private v1287 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1286[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1286&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1287[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1287[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1287&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1287 and iOS 1.0.380 (380)',()=>{
  assert.match(privateApp,/const APP_VER='v1287 · 抖音全屏作品与配乐（私人）'/);

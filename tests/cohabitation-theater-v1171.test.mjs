...
test('private v1323 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1322[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1322&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1323[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1323[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1323&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1323 and iOS 1.0.395 (395)',()=>{
  assert.match(privateApp,/const APP_VER='v1323 · 气泡即时配色与角色偏好、情侣任务和收款回执'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.395 \(395\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 395;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.395;/g)||[]).length>=12);
});

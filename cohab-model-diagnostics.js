...
window.cohabModelDiagnosticOpen=function(id){
  ...
  const history=rows.map(row=>'<div class="section">'+field(row.kind==='host'?'主角':'配角',row.actorName)+field('请求时间',fmtDT(row.at))+field('请求阶段',row.stage)+field('当时请求配置',row.routeName+' · '+row.slot+' · '+row.model)+field('请求输出上限',row.maxTokens+' tokens')+field('请求规模',row.messageCount+' 条 · '+row.requestChars+' 字符')+field('状态',row.status+(row.httpStatus?' · HTTP '+row.httpStatus:''))+(row.responses||[]).map((r,i)=>field(i?'续写结束原因':'首次结束原因',r.reason)+field(i?'续写输出上限':'首次输出上限',r.maxTokens+' tokens')+(r.model?field('上游返回模型',r.model):'')+(r.outputTokens!=null?field('上游输出用量',r.outputTokens+' tokens'):'')).join('')+(row.returnedChars!=null?field('返回文本长度',row.returnedChars+' 字符'):'')+'</div>').join('');

...
DELIVERY = ROOT / "delivery-v1031"
PACKAGE_NAME = "SmallPhone_v1031_WeChatHome_iOS152_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百五十二次安装_v1031_微信首页视觉升级_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1031":
...
if "APP_VER='v1031 · 微信首页视觉升级';" not in app_text:
    raise RuntimeError("bundled web version is not v1031")
project_text = (STAGING / "PhoneCompanionTest.xcodeproj" / "project.pbxproj").read_text(
    encoding="utf-8"
)
if project_text.count("CURRENT_PROJECT_VERSION = 152;") != 12:
    raise RuntimeError("private iOS build number is not consistently 152")
if project_text.count("MARKETING_VERSION = 1.0.152;") != 12:
    raise RuntimeError("private iOS marketing version is not consistently 1.0.152")

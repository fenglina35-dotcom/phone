...
test('v1178 shared theater asset is byte-identical and loaded after app core',()=>{
  ...
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1252[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1252[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1252&r=v1184-ios-web-crash-cohab-turn-keyboard-1"/);
...
test('private artifact identity advances independently to v1252 and iOS 1.0.361 (361)',()=>{
  assert.match(privateApp,/const APP_VER='v1252 · 私人同步网页八项修复'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=336/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.361 \(361\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 361;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.361;/g)||[]).length>=12);
});

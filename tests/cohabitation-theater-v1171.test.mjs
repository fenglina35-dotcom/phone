...
test('private v1293 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1292[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1292&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1293[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1293[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1293&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1293 and iOS 1.0.380 (380)',()=>{
  assert.match(privateApp,/const APP_VER='v1293 · 抖音边栏统一与双击点赞（私人）'/);

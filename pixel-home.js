...
function renderPixelHome(){
  ...
  return '<iframe id="pixel-home-frame" title="像素少女" allow="autoplay; fullscreen" style="position:absolute;inset:0;width:100%;height:100%;border:0;background:#eee3d5" src="games/pixel-home/index.html?v=1202&amp;session='+token+'"></iframe>';
}
...
async function pixelHomePlan(s,manual=false,stats){
  ...
  if(!pixelHomeValid(s))throw new Error('情侣绑定已改变，请重新进入小屋。');
  const task=(async()=>{
    await saveNowAsync();if(!pixelHomeValid(s))throw new Error('照顾已停止。');
    ...
    const raw=await chatAPI([{role:'system',content:prompt},{role:'user',content:JSON.stringify({dates,state:Object.fromEntries(['mood','food','energy','health','clean','sleeping'].map(k=>[k,state[k]]))})}],{routeIndex:roleChatRouteIndex(c),aux:c.model==='aux',independentRoleModel:true,complete:true,max:400,timeout:70000});
    ...
    entry.plan=Object.fromEntries(Object.entries(entry.plan).filter(([d])=>d>=today).slice(0,14));entry.planError='';await saveNowAsync();
    s.recent={value:result,at:Date.now(),used:manual};return result;
  })();s.pending=task;
  try{return await task;}catch(e){if(pixelHomeValid(s)){entry.planError=dates.every(d=>entry.plan?.[d]===0||entry.plan?.[d]===1)?'':String(e.message||e).slice(0,160);await saveNowAsync();}throw e;}finally{s.pending=null;}
}
async function pixelHomeMorning(s){
  const entry=pixelHomeEntry(s),due=PixelHomePolicy.due(entry);if(due){entry.appliedDay=due.day;entry.look=due.look;if(entry.state)entry.state.look=due.look;await saveNowAsync();}

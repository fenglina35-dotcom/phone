...
test('private v1325 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1324[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1324&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1325[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1325[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1325&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1325 and iOS 1.0.396 (396)',()=>{
  assert.match(privateApp,/const APP_VER='v1325 · 电话频率入口与手机显示修复'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.396 \(396\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 396;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.396;/g)||[]).length>=12);
});

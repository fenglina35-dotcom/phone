...
function fixture(source=code,opt={}){
 const data={settings:{},contacts:[{id:'test'}],image:'data:image/png;base64,'+'A'.repeat(700000)},blob=new Blob([JSON.stringify(data)]),store=new Map(),calls=[],notices=[];let cloud=null,parts=[],offset=0,commits=0,fullStateCalls=0,mirrorUploads=0,fail=false;
 ...
 privatePhoneAccountCall:async(action,payload={},timeoutMs)=>{calls.push({action,bytes:JSON.stringify(payload).length,timeoutMs});if(action==='account.backup.info')return {ok:true,found:!!cloud,backup:cloud};if(action==='account.backup.upload')throw new Error('bridge oversized payload');if(action.endsWith('.begin')){parts=[];offset=0;return {ok:true,token:'fixture'};}if(action.endsWith('.chunk')){if(fail)throw new Error('chunk failed');assert.equal(payload.offset,offset);const bytes=Buffer.from(payload.base64,'base64');assert(bytes.length<=192*1024);parts.push(bytes);offset+=bytes.length;return {ok:true};}if(action.endsWith('.commit')){if(opt.requireCommitTimeout&&!(timeoutMs>=660000))throw new Error('outer web timeout expired before native upload');commits++;assert.deepEqual(JSON.parse(Buffer.concat(parts).toString()),data);return {ok:true,saved:true};}return {ok:true};}};
 c.window=c;vm.createContext(c);vm.runInContext(source,c);return {c,calls,notices,store,get commits(){return commits;},get fullStateCalls(){return fullStateCalls;},get mirrorUploads(){return mirrorUploads;},setCloud(v){cloud=v;},fail(){fail=true;}};
}

...
test('v1189 private identifiers retain the performance chain while public stays v1184', () => {
  ...
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1202'/);
  assert.match(index, /app\.js\?v=1202&r=v1202-couple-watch-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=323/);
  assert.match(app, /APP_VER='v1202 · 像素少女照顾版'/);
  assert.match(overlay, /323-pixel-home-1/);
  assert.match(webview, /1\.0\.323 \(323\)/);
  assert.match(bridge, /private static let build = "1\.0\.323 \(323\)"/);
  assert.match(bridge, /static let contractVersion = 35/);
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 323;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.323;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1202 · 像素少女照顾版'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1202'/);
...
test('current Mac guides identify private v1202 and web not deployed', () => {
  const install = fs.readFileSync(
    path.join(project, '第三百二十三次安装_v1202_像素少女_请先读.md'),
    'utf8',
  );
  const mac = fs.readFileSync(path.join(project, '请在Mac编译前先读.md'), 'utf8');
  for (const guide of [install, mac]) {
    assert.match(guide, /v1202/);
    assert.match(guide, /网页.*推送/);
    assert.match(guide, /1\.0\.323 \(323\)/);
  ...
  assert.match(mac, /当前候选：网页源码与私人内置网页 v1202/);
  assert.match(mac, /网页.*推送/);
  assert.match(install, /网页源码与私人内置网页均为 v1202/);

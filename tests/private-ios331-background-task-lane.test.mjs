...
test('private performance candidate keeps v1278 while public web advances independently', () => {
  assert.equal(privateIndex, privateAlias);
  assert.match(privateIndex, /window\.__NORTH_SHELL_BUILD__='1278'/);
  assert.match(privateIndex, /app\.js\?v=1278&r=v1278-private-backup-storage-1/);
  assert.match(privateIndex, /private-runtime-diagnostics\.js\?v=339/);
  assert.match(privateRepair, /index\.html\?repair=1&v=1278/);

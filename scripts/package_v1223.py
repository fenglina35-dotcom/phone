"""Separate shared web release and private Mac source; archive committed files only."""
...
def validate(web,private):
 ...
 for s in [html,phtml]:assert "__NORTH_SHELL_BUILD__='1223'" in s and 'app.js?v=1223' in s
 assert "BUILD='1223'" in text(web['sw.js'])
 ...
 assert 'private-smart-air.js?v=1223' in phtml
 ...
 assert project.count('CURRENT_PROJECT_VERSION = 346;')==12 and project.count('MARKETING_VERSION = 1.0.346;')==12
...
def main():
 ...
 for scope,files,name,prefix in [('网页',web,'小手机_v1223_网页版_聊天重新生成修复.zip','SmallPhone_v1223_Web/'),('私人',private,'小手机_v1223_私人版_iOS346_聊天与智能家居汇总_Mac待编译源码包.zip','SmallPhone_v1223_iOS346_MacSource/')]:
  files['SOURCE_COMMIT.txt']=f'commit={head}\nversion=v1223\nscope={scope}\nios=1.0.346 (346)\nbridge=37\nprivate-cloud-avatar-migration=VERIFIED-2026-09-09\nmac-build-verified=no\nreal-iphone-verified=no\nkind=source-only-not-IPA\nweb-deployment=verify-separately\ndaily-event-ledger=not-included\n'.encode()

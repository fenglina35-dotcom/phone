...
test('private identifiers remain v1327 and iOS 394 while public advances independently', () => {
  assert.equal(index, alias);
  assert.match(index, /window\.__NORTH_SHELL_BUILD__='1327'/);
  assert.match(index, /app\.js\?v=1327&r=v1327-private-delivery-tag-1/);
  assert.match(index, /private-runtime-diagnostics\.js\?v=339/);
  assert.match(app, /APP_VER='v1327 · 外卖嵌套商品名解析修复'/);
  ...
  assert.equal((pbx.match(/CURRENT_PROJECT_VERSION = 397;/g) || []).length, 12);
  assert.equal((pbx.match(/MARKETING_VERSION = 1\.0\.397;/g) || []).length, 12);
  assert.match(publicApp, /APP_VER='v1326 · 外卖嵌套商品名解析修复'/);
  assert.match(publicEntry, /window\.__NORTH_SHELL_BUILD__='1326'/);
...
test('Mac guide identifies the current private v1327 iOS397 source', () => {

...
test('v1178 shared theater asset is byte-identical and loaded after app core',()=>{
  ...
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1217[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1216[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1216&r=v1184-ios-web-crash-cohab-turn-keyboard-1"/);
...
test('private artifact identity advances independently to web core v1216 and iOS 1.0.342 (342)',()=>{
  assert.match(privateApp,/const APP_VER='v1217 · 私人智能空调版'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=333/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.342 \(342\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 342;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1\.0\.342;/g)||[]).length>=12);
});

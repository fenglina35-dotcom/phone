...
test('private identity remains v1274 and iOS 373 while public advances independently', () => {
  assert.match(privateApp, /__NORTH_SHELL_BUILD__!=='1274'/);
  ...
  for (const html of [privateIndex, privateAlias]) {
    ...
    assert.match(html, /app\.js\?v=1274&r=v1274-private-sync-1/);
    assert.match(html, /private-runtime-diagnostics\.js\?v=339/);
  }
  assert.match(privateRepair, /index\.html\?repair=1&v=1274/);
  assert.match(swift, /1\.0\.373 \(373\)/);

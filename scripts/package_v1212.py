...
def validate(web,private):
    ...
    for token in ["APP_VER='v1212",'modelOutputUnfilteredToggle','lifeNoteCommitReply','roleSocialIdentityPin','proactiveContinuationContext']:
    ...
    assert 'private-smart-lock.js?v=338' in pshell and 'private-smart-lock.css?v=338' in pshell
    ...
    assert 'request-diagnostics.js?v=1212' in shell and 'request-diagnostics.js?v=1212' in pshell
    ...
    assert project.count('CURRENT_PROJECT_VERSION = 338;')==12
    assert project.count('MARKETING_VERSION = 1.0.338;')==12
    assert 'static let contractVersion = 36' in private['PhoneCompanionTest/PhoneNativeBridge.swift'].decode('utf8')
    assert "const BUILD='1212'" in web['sw.js'].decode('utf8')
    assert "__NORTH_SHELL_BUILD__='1212'" in shell and "__NORTH_SHELL_BUILD__='1212'" in pshell
    ...
    assert 'message-beijing-time.js?v=1212' in shell and 'message-beijing-time.js?v=1212' in pshell
...
def main():
    ...
    results=[]
    for kind,files,title in [('网页',web,'SmallPhone_v1212_Web'),('私人',private,'SmallPhone_v1212_iOS338_MacSource')]:
        if kind=='私人':files={p:b for p,b in files.items() if not (len(PurePosixPath(p).parts)==1 and '安装' in p and p!=GUIDE)}
        name='小手机_v1212_网页版_请求诊断与简洁时间.zip' if kind=='网页' else '小手机_v1212_私人版_iOS338_请求诊断与简洁时间_Mac待编译源码包.zip'

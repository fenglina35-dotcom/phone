...
(async () => {
  ...
  function applyMorning(info){if(!info)return;if(info.look===0||info.look===1){if(info.day&&state.wardrobeDay!==info.day){state.look=info.look;state.wardrobeDay=info.day;window.RoseWardrobe?.applyLook(info.look);save();}if(drawer==='wardrobe')renderPanel();}if(info.error&&info.error!==lastMorningError)say('今天的穿搭安排未更新，保留现有衣服。可点让他照顾再试。',6500);lastMorningError=info.error||'';}

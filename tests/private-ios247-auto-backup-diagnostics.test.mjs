...
test('private iOS 310 overlay owns the disable marker and cancels automatic scheduling', async () => {
...
test('manual backup and both restore actions remain free of the automatic-disable guard', () => {
  for (const name of [
    ...
    assert.doesNotMatch(source, /310-release-v1184-v1/);
  }
  assert.doesNotMatch(
    overlay,
...
test('native recovery UI stays outside WebKit and carries the private 310 identity', () => {
  ...
  assert.match(webView, /smallPhone\.webContentTerminationTimes\.v19\.build310/);

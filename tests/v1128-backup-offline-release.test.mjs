...
test('v1178 has a unique visible identity across every public entry and cache layer',()=>{
  assert.match(app,/__NORTH_SHELL_BUILD__!==\'1196\'/);
  assert.match(app,/APP_VER='v1196 · 内心格式本地稳定版'/);
  assert.match(shell,/__NORTH_SHELL_BUILD__='1196'/);
  assert.match(shell,/app\.js\?v=1196&r=v1196-couple-watch-trigger-1/);
  assert.match(index,/小手机\.html\?v=1196/);
  assert.match(repair,/小手机\.html\?v=1196/);
  assert.match(worker,/const BUILD='1196'/);
  assert.match(worker,/north-shell-v1196-couple-watch-trigger-1/);
  assert.match(hotfix,/sw\.js\?v=1196&r=v1196-couple-watch-trigger-1/);

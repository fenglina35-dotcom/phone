 ...
 try{
  await Promise.all(['chat','douyin'].map(async kind=>{
   ...
   await page.evaluate(({kind,origin})=>{
    ...
    window.watchTestOpen=()=>{if(kind==='chat')go('chat',{id:'watch-other'});else openApp('douyin');};
    window.watchTestOpen();window.watchTestStarted=Date.now();
   },{kind,origin});
   console.log(kind+': entered actual page, checking short visit');
   await page.waitForTimeout(kind==='chat'?29000:19000);
   ...
   console.log(kind+': reentered, waiting for actual 30–60 / 20–60 second timer');
   ...
   assert(elapsed>=(kind==='chat'?30000:20000)&&elapsed<63000,'threshold outside expected window: '+elapsed);
   // Returning to partner chat after detection must not cancel the HTTP result.
   await page.evaluate(()=>go('chat',{id:S.couple.cid}));
   ...
   assert.equal(await page.getByText('刚才去找朋友了呀。',{exact:true}).count(),1);
   assert.deepEqual(errors,[]);
   console.log(JSON.stringify({kind,elapsed,modelRequests:1,delivered:4,afterNavigation:true,version:result.version,pageErrors:errors}));
   await context.close();
  }));

...
test('private entries carry diagnostics 334 and every iOS target carries build 349', () => {
  ...
  assert.match(bridge, /1\.0\.349 \(349\)/);
  assert.equal(
    (project.match(/CURRENT_PROJECT_VERSION = 349;/g) || []).length,
    12
  );
  assert.equal(
    (project.match(/MARKETING_VERSION = 1\.0\.349;/g) || []).length,
    12
  );
});

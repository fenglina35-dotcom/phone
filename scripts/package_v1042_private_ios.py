...
DELIVERY = ROOT / "delivery-v1042"
PACKAGE_NAME = "SmallPhone_v1042_MemoryDeliveryCaptcha_iOS161_MacReady"
STAGING = DELIVERY / PACKAGE_NAME
ZIP_PATH = DELIVERY / f"{PACKAGE_NAME}.zip"
INSTALL_GUIDE = "第一百六十一次安装_v1042_跨场景记忆与外卖验证续跑_请先读.md"
...
if DELIVERY.exists():
    ...
    if resolved.parent != ROOT.resolve() or resolved.name != "delivery-v1042":
...
if "APP_VER='v1042 · 跨场景记忆与外卖验证续跑修复版';" not in app_text:
    raise RuntimeError("bundled web version is not v1042")
for required_text in [
    "chat-quote-pending",
    "cinemaRetryOriginalVideo",
    "cinemaDirectPlayOnly",
    "stack[stack.length-2].p==='gameshub'",
]:
    if required_text not in app_text:
        raise RuntimeError(f"v1042 bundled repair is missing: {required_text}")

theme_text = (bundle / "glass-theme.css").read_text(encoding="utf-8")
for required_text in [
    ".chat-quote-pending",
    ".wx-chat-premium:has(>.chat-quote-pending)>.manual-reply-row",
]:
    if required_text not in theme_text:
        raise RuntimeError(f"v1042 quote layout is missing: {required_text}")

info_text = (bundle / "Info.plist").read_text(encoding="utf-8")
if "<string>1042</string>" not in info_text:
    raise RuntimeError("PhoneWeb.bundle Info.plist is not v1042")

wechat_text = (bundle / "wechat-me.js").read_text(encoding="utf-8")
for required_text in [
    "小手机智能客服",
    "角色心情标签在哪里开启",
    "wxSupportMatch",
]:
    if required_text not in wechat_text:
        raise RuntimeError(f"WeChat support source is missing: {required_text}")
...
for required_text in [
    ...
    "var roleRequests={}",
    "deliveryRolePreludeAllowed",
    "支付宝官方付款二维码",
    "r.roleAttempts[cid]=attempt",
    "lastUserAt<=attempt.endedAt",
]:
...
if project_text.count("CURRENT_PROJECT_VERSION = 161;") != 12:
    raise RuntimeError("private iOS build number is not consistently 161")
if project_text.count("MARKETING_VERSION = 1.0.161;") != 12:
    raise RuntimeError("private iOS marketing version is not consistently 1.0.161")

webview_text = (
    STAGING / "PhoneCompanionTest" / "LocalPhoneWebView.swift"
).read_text(encoding="utf-8")
if "__SMALL_PHONE_PRIVATE_BUILD__ = '1.0.161 (161)'" not in webview_text:
    raise RuntimeError("private build marker is not 1.0.161 (161)")
if "contractVersion = 25" not in (

...
test('private v1295 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1294[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1294&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1295[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1295[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1295&r=v1274-web-cohab-guests-1"/);

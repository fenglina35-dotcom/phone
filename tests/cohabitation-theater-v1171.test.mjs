...
test('private v1311 inherits the current web theater without dropping its private shell',()=>{
  assert.equal(bundleTheater,theater);
  assert.match(bundleTheater,/guest2|ct_wechat_enabled/);
  assert.match(webHtml,/app\.js\?v=1310[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1310&r=v1274-web-cohab-guests-1"/);
  for(const html of [privateHtml,privateAlias])assert.match(html,/app\.js\?v=1311[^\n]*<\/script>\s*<script src="private-reply-intercept\.js\?v=1311[^\n]*<\/script>\s*<script src="cohab-theater\.js\?v=1311&r=v1274-web-cohab-guests-1"/);
...
test('private artifact identity advances independently to v1311 and iOS 1.0.389 (389)',()=>{
  assert.match(privateApp,/const APP_VER='v1311 · 每日一封信、地图跟资料、通话中文动作、不许预演（私人）'/);
  assert.match(privateHtml,/private-runtime-diagnostics\.js\?v=339/);
  assert.match(read('native/private-small-phone/XcodeProject/PhoneCompanionTest/PhoneNativeBridge.swift'),/1\.0\.389 \(389\)/);
  const project=read('native/private-small-phone/XcodeProject/PhoneCompanionTest.xcodeproj/project.pbxproj');
  assert.ok((project.match(/CURRENT_PROJECT_VERSION = 389;/g)||[]).length>=12);
  assert.ok((project.match(/MARKETING_VERSION = 1.0.389;/g)||[]).length>=12);
});

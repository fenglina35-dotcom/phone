...
(async()=>{
 ...
 try{for(const privateApp of [true,false]){
  ...
  if(!privateApp){await page.evaluate(async()=>{await navigator.serviceWorker.register('sw.js?v=1244&r=backup-browser-test',{updateViaCache:'none'});await navigator.serviceWorker.ready;});await page.waitForFunction(()=>!!navigator.serviceWorker.controller,{timeout:60000});}

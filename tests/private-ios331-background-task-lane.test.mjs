...
test('private performance candidate has a v1238 identity while public web stays v1238', () => {
  assert.equal(privateIndex, privateAlias);
  assert.match(privateIndex, /window\.__NORTH_SHELL_BUILD__='1238'/);
  assert.match(privateIndex, /app\.js\?v=1238&r=v1238-backup-work-budget-1/);
  assert.match(privateIndex, /private-runtime-diagnostics\.js\?v=335/);
  assert.match(privateRepair, /index\.html\?repair=1&v=1238/);
  assert.match(privateApp, /APP_VER='v1238 · 备份等待与导入时序修复'/);
